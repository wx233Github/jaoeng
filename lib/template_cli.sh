...
tm_validate_args() {
	...
	for arg in "$@"; do
		...
		case "$arg" in
		-h | --help | --cron | --non-interactive | --check | --audit-only | --cf-ip-update | --dry-run | --preflight | --template-dry-run | --template-precheck | --fail-fast | --continue-on-error | --json | --quiet | --template-impact-report | --template-audit-report) ;;
