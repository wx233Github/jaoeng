#!/bin/bash
# 🚀 Docker 自动更新助手
# v2.14.4 真正完整版：补全所有功能细节，并集成所有框架兼容性修复。
# 功能：
...
configure_watchtower_settings() {
    ...
    if [[ "$label_choice" =~ ^[Yy]$ ]]; then
        read -p "请输入筛选标签 (例如: com.centurylabs.watchtower.enable=true): " WATCHTOWER_LABELS_NEW
    ...
    if [[ "$extra_args_choice" =~ ^[Yy]$ ]]; then
        read -p "请输入额外参数 (例如: --no-startup-message): " WATCHTOWER_EXTRA_ARGS_NEW
        WATCHTOWER_EXTRA_ARGS="${WATCHTOWER_EXTRA_ARGS_NEW:-}"
    else
        WATCHTOWER_EXTRA_ARGS=""
    fi
    read -p "是否启用 Watchtower 调试模式？(y/n) (当前: $([ "$WATCHTOWER_DEBUG_ENABLED" = "true" ] && echo "是" || echo "否")): " debug_choice
...
configure_cron_task() {
    ...
    cat > "$CRON_UPDATE_SCRIPT" <<EOF
#!/bin/bash
PROJECT_DIR='${DOCKER_COMPOSE_PROJECT_DIR_CRON}'
LOG_FILE='${LOG_FILE}'
echo "\$(date '+%Y-%m-%d %H:%M:%S') - Starting update..." >> "\$LOG_FILE" 2>&1
...
view_and_edit_config() {
    ...
    save_config
    echo -e "${COLOR_YELLOW}ℹ️ 配置已更新。部分更改可能需要通过主菜单选项重新设置任务才能生效。${COLOR_RESET}"
    press_enter_to_continue
    return 0
}
