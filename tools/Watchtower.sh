...
_print_header() {
    ...
    local box_width; box_width=$(expr $title_width + 4)
    if [ $box_width -lt 62 ]; then box_width=62; fi
...
show_container_info() {
    while true; do 
        ...
        while IFS='|' read -r name image status; do 
            ...
            printf "%-5s %-25.25s %-45.45s %b\n" "$i" "$name" "$image" "$status_colored"; i=$(expr $i + 1); 
...
view_and_edit_config(){ 
    while true; do 
        ...
        if echo "$choice" | grep -qE '^[0-9]+$' && [ "$choice" -ge 1 ] && [ "$choice" -le 10 ]; then
            sleep 0.5;
        fi
    done
}
