...
WATCHTOWER_CONFIG_SELF_UPDATE_MODE="${WATCHTOWER_CONFIG_SELF_UPDATE_MODE:-false}" # This line is kept for compatibility but is not used in new functions
...
show_container_info() {
  while true; do
    ...
    case "$choice" in
      q|Q|"")
        return # Changed from exit 10 to return for better control flow inside the same script
        ;;
      *)
        if ! [[ "$choice" =~ ^[0-9]+$ ]]; then
  ...
  while true; do
    ...
    case "$MANAGE_CHOICE" in
      ...
      2)
        ...
        ;;
      3)
        if systemctl list-timers | grep -q "docker-compose-update.timer"; then
          if confirm_action "确定移除 Systemd Timer 吗？"; then
            systemctl disable --now docker-compose-update.timer &>/dev/null
            rm -f /etc/systemd/system/docker-compose-update.{service,timer}
            systemctl daemon-reload
            log_info "Systemd Timer 已移除。"
          fi
        else
          echo -e "${COLOR_YELLOW}ℹ️ 未检测到由本脚本配置的 Systemd Timer。${COLOR_RESET}"
        fi
        press_enter_to_continue
        ;;
      4)
...
run_watchtower_once(){
  ...
  if docker ps --format '{{.Names}}' | grep -q '^watchtower$'; then
    ...
    if ! confirm_action "是否继续运行一次性更新？"; then echo -e "${COLOR_YELLOW}已取消。${COLOR_RESET}"; return 0; fi
  fi
...
update_menu(){
  while true; do
    ...
    case "$c" in
      ...
      2) configure_systemd_timer; break ;;
      3) configure_cron_task; break ;;
      q|Q|"") break ;;
...
main_menu(){
  while true; do
    ...
    echo
    echo "主菜单选项："
    echo "1) 🔄 设置更新模式 (Watchtower / Cron / Systemd)"
    echo "2) 📋 交互式容器管理"
    echo "3) 🔔 配置通知"
    echo "4) ⚙️ 任务管理"
    echo "5) 📝 查看/编辑配置"
    ...
    case "$choice" in
      ...
      3) configure_notify; press_enter_to_continue ;;
      4) manage_tasks ;;
