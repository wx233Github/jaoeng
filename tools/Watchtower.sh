...
_start_watchtower_container_logic(){
  ...
  if (( ${#containers_to_monitor[@]} > 0 )); then cmd_parts+=("${containers_to_monitor[@]}"); fi
  echo -e "${COLOR_CYAN}执行命令: ${cmd_parts[*]} ${COLOR_RESET}"; set +e; "${cmd_parts[@]}"; local rc=$?; set -e
  if [[ "$mode_description" == "一次性更新" ]]; then if (( rc == 0 )); then echo -e "${COLOR_GREEN}✅ $mode_description 完成。${COLOR_RESET}"; else echo -e "${COLOR_RED}❌ $mode_description 失败。${COLOR_RESET}"; fi; return $rc
...
configure_exclusion_list() {
    ...
    while true; do
        ...
        local items_str=""
        local i=0
        while (( i < ${#all_containers[@]} )); do
            local container="${all_containers[$i]}"
            local is_excluded=" "
            if [[ -v excluded_map["$container"] ]]; then
                is_excluded="✔"
            fi
            items_str="$items_str\n  $((i+1)). [${COLOR_GREEN}${is_excluded}${COLOR_RESET}] $container"
            ((i++))
        done
        items_str=${items_str#\\n} 
...
view_and_edit_config(){ 
    while true; do 
        ...
        case "$choice" in 
            ...
            10) read -r -p "启用 Cron？(y/n): " d; if [[ "$d" =~ ^[Yy]$ ]]; then CRON_TASK_ENABLED="true"; else CRON_TASK_ENABLED="false"; fi; save_config ;; 
            "") return ;; 
