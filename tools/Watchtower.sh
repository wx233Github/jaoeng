...
view_and_edit_config(){ 
    while true; do 
        if [[ "${JB_ENABLE_AUTO_CLEAR}" == "true" ]]; then clear; fi; load_config; _print_header "⚙️ 配置查看与编辑 ⚙️"; local val; local color;
        val="${TG_BOT_TOKEN:-}"; if [ -n "$val" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf " 1. TG Token: %b%s%b\n" "$color" "$([ -n "$val" ] && echo "已设置" || echo "未设置")" "$COLOR_RESET"
        val="${TG_CHAT_ID:-}"; if [ -n "$val" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf " 2. TG Chat ID:   %b%s%b\n" "$color" "${val:-未设置}" "$COLOR_RESET"
        val="${EMAIL_TO:-}"; if [ -n "$val" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf " 3. Email:     %b%s%b\n" "$color" "${val:-未设置}" "$COLOR_RESET"
        val="${WATCHTOWER_EXTRA_ARGS:-}"; if [ -n "$val" ]; then color="$COLOR_GREEN"; else color="$COLOR_CYAN"; fi; printf " 4. 额外参数: %b%s%b\n" "$color" "${val:-无}" "$COLOR_RESET"
        val="${WATCHTOWER_DEBUG_ENABLED:-false}"; if [ "$val" = "true" ]; then color="$COLOR_GREEN"; else color="$COLOR_CYAN"; fi; printf " 5. 调试模式: %b%s%b\n" "$color" "$([ "$val" = "true" ] && echo "是" || echo "否")" "$COLOR_RESET"
        val="$(_format_seconds_to_human "${WATCHTOWER_CONFIG_INTERVAL:-}")"; if [ "$val" != "N/A" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf " 6. 间隔: %b%s%b\n" "$color" "$([ "$val" != "N/A" ] && echo "$val" || echo "未设置")" "$COLOR_RESET"
        val="${WATCHTOWER_ENABLED:-false}"; if [ "$val" = "true" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf " 7. 启用状态: %b%s%b\n" "$color" "$([ "$val" = "true" ] && echo "是" || echo "否")" "$COLOR_RESET"
        val="${CRON_HOUR:-}"; if [ -n "$val" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf " 8. Cron 小时:      %b%s%b\n" "$color" "${val:-未设置}" "$COLOR_RESET"
        val="${DOCKER_COMPOSE_PROJECT_DIR_CRON:-}"; if [ -n "$val" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf " 9. Cron 目录: %b%s%b\n" "$color" "${val:-未设置}" "$COLOR_RESET"
        val="${CRON_TASK_ENABLED:-false}"; if [ "$val" = "true" ]; then color="$COLOR_GREEN"; else color="$COLOR_RED"; fi; printf "10. Cron 状态: %b%s%b\n" "$color" "$([ "$val" = "true" ] && echo "是" || echo "否")" "$COLOR_RESET"
