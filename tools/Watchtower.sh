#!/bin/bash
# =============================================================
# 🚀 Docker 自动更新助手 (v4.3.3 - Refactored with utils.sh)
# =============================================================
...
if [ -f "$UTILS_PATH" ]; then
    ...
    # Fallback a simple logger if utils.sh is not found
    log_err() { echo "[错误] $*" >&2; }
...
_format_seconds_to_human() { 
    local seconds="$1"
    if ! echo "$seconds" | grep -qE '^[0-9]+$'; then echo "N/A"; return; fi
    if [ "$seconds" -lt 3600 ]; then 
        echo "${seconds}s"
    else 
        local hours; hours=$(expr $seconds / 3600)
        echo "${hours}h"
    fi
}
...
show_container_info() { while true; do if [ "${JB_ENABLE_AUTO_CLEAR}" = "true" ]; then clear; fi; local header_line; header_line=$(printf "%-5s %-25s %-45s %-20s" "编号" "名称" "镜像" "状态"); local content_lines="$header_line"; local containers=(); local i=1; while IFS='|' read -r name image status; do containers+=("$name"); local status_colored="$status"; if echo "$status" | grep -qE '^Up'; then status_colored="${GREEN}运行中${NC}"; elif echo "$status" | grep -qE '^Exited|Created'; then status_colored="${RED}已退出${NC}"; else status_colored="${YELLOW}${status}${NC}"; fi; content_lines="$content_lines\n$(printf "%-5s %-25.25s %-45.45s %b" "$i" "$name" "$image" "$status_colored")"; i=$(expr $i + 1); done < <(docker ps -a --format '{{.Names}}|{{.Image}}|{{.Status}}'); content_lines="$content_lines\n\n a. 全部启动 (Start All)   s. 全部停止 (Stop All)"; _render_menu "📋 容器管理 📋" "$content_lines"; read -r -p " └──> 输入编号管理, 'a'/'s' 批量操作, 或按 Enter 返回: " choice; case "$choice" in "") return ;; a|A) if confirm_action "确定要启动所有已停止的容器吗?"; then log_info "正在启动..."; local stopped_containers; stopped_containers=$(docker ps -aq -f status=exited); if [ -n "$stopped_containers" ]; then docker start $stopped_containers &>/dev/null || true; fi; log_success "操作完成。"; press_enter_to_continue; else log_info "操作已取消。"; fi ;; s|S) if confirm_action "警告: 确定要停止所有正在运行的容器吗?"; then log_info "正在停止..."; local running_containers; running_containers=$(docker ps -q); if [ -n "$running_containers" ]; then docker stop $running_containers &>/dev/null || true; fi; log_success "操作完成。"; else log_info "操作已取消。"; fi ;; *) if ! [[ "$choice" =~ ^[0-9]+$ ]] || [ "$choice" -lt 1 ] || [ "$choice" -gt ${#containers[@]} ]; then log_warn "无效输入或编号超范围。"; sleep 1; continue; fi; local selected_container="${containers[$(expr $choice - 1)]}"; if [ "${JB_ENABLE_AUTO_CLEAR}" = "true" ]; then clear; fi; local action_items; action_items=$(cat <<-EOF
  ...
  6. › 进入容器 (Exec)
EOF
); _render_menu "操作容器: ${selected_container}" "$action_items"; read -r -p " └──> 请选择, 或按 Enter 返回: " action; case "$action" in 1) echo -e "${YELLOW}日志 (Ctrl+C 停止)...${NC}"; trap '' INT; docker logs -f --tail 100 "$selected_container" || true; trap 'echo -e "\n操作被中断。"; exit 10' INT; press_enter_to_continue ;; 2) echo "重启中..."; if docker restart "$selected_container"; then echo -e "${GREEN}✅ 成功。${NC}"; else echo -e "${RED}❌ 失败。${NC}"; fi; sleep 1 ;; 3) echo "停止中..."; if docker stop "$selected_container"; then echo -e "${GREEN}✅ 成功。${NC}"; else echo -e "${RED}❌ 失败。${NC}"; fi; sleep 1 ;; 4) if confirm_action "警告: 这将永久删除 '${selected_container}'！"; then echo "删除中..."; if docker rm -f "$selected_container"; then echo -e "${GREEN}✅ 成功。${NC}"; else echo -e "${RED}❌ 失败。${NC}"; fi; sleep 1; else echo "已取消。"; fi ;; 5) _print_header "容器详情: ${selected_container}"; (docker inspect "$selected_container" | jq '.' 2>/dev/null || docker inspect "$selected_container") | less -R; ;; 6) if [ "$(docker inspect --format '{{.State.Status}}' "$selected_container")" != "running" ]; then log_warn "容器未在运行，无法进入。"; else log_info "尝试进入容器... (输入 'exit' 退出)"; docker exec -it "$selected_container" /bin/sh -c "[ -x /bin/bash ] && /bin/bash || /bin/sh" || true; fi; press_enter_to_continue ;; *) ;; esac ;; esac; done; }
...
show_watchtower_details(){
    while true; do
        ...
        local content_lines_array=( "上次活动: $(get_last_session_time :- "未检测到")" "下次检查: $countdown" )
        ...
        local max_width=0; max_width=$(_get_visual_width "$title")
        for line in "${content_lines_array[@]}"; do local line_width; line_width=$(_get_visual_width "$line"); if [ "$line_width" -gt "$max_width" ]; then max_width=$line_width; fi; done
...
main_menu(){
    ...
    status_lines+=("📦 容器概览: 总计 $TOTAL (${GREEN}运行中 ${RUNNING}${NC}, ${RED}已停止 ${STOPPED}${NC})")
    if [ -n "$FINAL_EXCLUDE_LIST" ]; then 
        status_lines+=("🚫 排除列表: ${YELLOW}${FINAL_EXCLUDE_LIST//,/, }${NC} (${CYAN}${FINAL_EXCLUDE_SOURCE}${NC})")
    fi
    if [ -n "$NOTIFY_STATUS" ]; then 
        status_lines+=("🔔 通知已启用: ${GREEN}${NOTIFY_STATUS}${NC}")
    fi
