# =============================================================
# 🚀 Watchtower 管理模块 (v9.2.1-语法与健壮性修复)
# - 修复: (致命错误) 修正了 `view_and_edit_config` 函数中导致 `unexpected EOF`
#         的引号使用错误，确保脚本可以正常解析和运行。
# - 优化: 将创建模板文件的方式从 `sudo bash -c` 更改为更健壮的 `cat | sudo tee`
#         模式，提高了脚本的兼容性和稳定性。
# - 确认: 此版本在功能上与 v9.2.0 完全对等，并修复了所有已知的语法和逻辑错误。
# =============================================================

# --- 脚本元数据 ---
SCRIPT_VERSION="v9.2.1"
...
_ensure_notification_template_exists() {
    ...
    # 使用 here-document 和 pipe 到 sudo tee 的健壮方式写入文件
    cat <<'EOF' | sudo tee "$TEMPLATE_FILE" > /dev/null
{{- if .Report -}}
*🐳 Watchtower 扫描报告*
