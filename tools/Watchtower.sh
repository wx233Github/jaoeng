...
if [ -f "$UTILS_PATH" ]; then
    ...
    log_err() { echo "[错误] $*" >&2; }
    log_err "致命错误: 通用工具库 $UTILS_PATH 未找到！"; exit 1
fi
...
main_menu(){
    ...
    local content_str=" 🕝 Watchtower 状态: ${STATUS_COLOR} (名称排除模式)\n ⏳ 下次检查: ${COUNTDOWN}\n 📦 容器概览: 总计 $TOTAL (${GREEN}运行中 ${RUNNING}${NC}, ${RED}已停止 ${STOPPED}${NC})"
    if [ -n "$FINAL_EXCLUDE_LIST" ]; then content_str+="\n 🚫 排除列表: ${YELLOW}${FINAL_EXCLUDE_LIST//,/, }${NC} (${CYAN}${FINAL_EXCLUDE_SOURCE}${NC})"; fi
    if [ -n "$NOTIFY_STATUS" ]; then content_str+="\n 🔔 通知已启用: ${GREEN}${NOTIFY_STATUS}${NC}"; fi
    content_str+="\n\n主菜单：\n  1. › 配置 Watchtower\n  2. › 配置通知\n  3. › 任务管理\n  4. › 查看/编辑配置 (底层)\n  5. › 手动更新所有容器\n  6. › 详情与管理"
