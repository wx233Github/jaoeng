#!/usr/bin/env bash
#
# Docker 自动更新助手 (v3.3.6 - 重构通知菜单并修复返回逻辑)
#
set -euo pipefail

export LC_ALL=C.utf8

VERSION="v3.3.6-final-ux-refactor"
...
notification_menu() {
    while true; do
        ...
        echo "1. 配置 Telegram  ($tg_status)"
        echo "2. 配置 Email      ($email_status)"
        echo "3. 发送测试通知"
        echo "4. 清空所有通知配置"
        echo "---"
        ...
        case "$choice" in
            ...
            3) 
                if [[ -z "$TG_BOT_TOKEN" && -z "$EMAIL_TO" ]]; then
                    ...
                    log_info "正在发送测试..."
                    send_notify "这是一条来自 Docker 助手 v${VERSION} 的测试消息。"
                    log_info "测试通知已发送 (请检查您的客户端)。"
                fi
...
main_menu(){
  while true; do
    ...
    local ret_code=0
    case "$choice" in
      1) configure_watchtower || ret_code=$?; if [[ "$ret_code" -ne 10 ]]; then press_enter_to_continue; fi ;;
      2) show_container_info; return 10 ;;
      3) notification_menu; return 10 ;;
