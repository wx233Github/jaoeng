#!/bin/bash
# =============================================================
# 🚀 Docker 自动更新助手 (v3.9.8 - Notify & Spacing Fix)
# =============================================================
...
send_notify() {
    ...
    if [ -n "$TG_BOT_TOKEN" ] && [ -n "$TG_CHAT_ID" ]; then
        (
            # 将消息中的特定字符进行URL编码以安全传输
            local encoded_message
            encoded_message=$(echo "$message" | sed \
                ...
                -e 's/;/%3b/g' -e 's/>/%3e/g' -e 's/?/%3f/g' -e 's/@/%40/g' \
                -e 's/\[/%5b/g' -e 's/\\/%5c/g' -e 's/\]/%5d/g' -e 's/\^/%5e/g' \
                -e 's/_/%5f/g' -e 's/`/%60/g' -e 's/{/%7b/g' -e 's/|/%7c/g' \
                -e 's/}/%7d/g' -e 's/~/
                
            local url="https://api.telegram.org/bot${TG_BOT_TOKEN}/sendMessage"
            local data="chat_id=${TG_CHAT_ID}&text=${encoded_message}&parse_mode=Markdown"
            
            curl -s -X POST "$url" -d "$data" >/dev/null 2>&1
        ) &
    fi

    # Email 通知 (如果需要可以取消注释并配置)
    # if [ -n "$EMAIL_TO" ] && command -v mail >/dev/null 2>&1; then
    #     (
    #         echo "$message" | mail -s "Docker Auto-Update Notification" "$EMAIL_TO"
    #     ) &
    # fi
}
# =============================================================
...
# =============================================================
# press_enter_to_continue 不再添加多余的空格
press_enter_to_continue() { read -r -p "$(echo -e "\n${COLOR_YELLOW}按 Enter 键继续...${COLOR_RESET}")"; }

# 此函数仅用于在对齐的标签中为CJK字符添加空格，不再影响其他地方
_add_spacing_to_cjk_label() {
    echo "$1" | sed 's/\([[:han:]]\)/\1 /g' | sed 's/ $//'
}

_render_aligned_lines() {
    local indent="$1"; shift; local lines=("$@"); local max_label_width=0
    for line in "${lines[@]}"; do
        local label_raw; label_raw=$(echo "$line" | cut -d':' -f1)
        # 仅对标签调用空格函数
        local label_spaced; label_spaced=$(_add_spacing_to_cjk_label "$label_raw")
        local width; width=$(_get_visual_width "$label_spaced")
    ...
    for line in "${lines[@]}"; do
        ...
        local label_spaced; label_spaced=$(_add_spacing_to_cjk_label "$label_raw")
        local current_label_width; current_label_width=$(_get_visual_width "$label_spaced")
        ...
        echo -e "${indent}${label_spaced}${padding} :${value}"
    done
}
# =============================================================
...
# 以下所有函数的代码保持不变，除了 `notification_menu` 中的一个调用...
_start_watchtower_container_logic(){
  ...
  local containers_to_monitor=(); if [ -n "$final_exclude_list" ]; then
      ...
      if [ -n "$included_containers" ]; then
          while IFS= read -r line; do containers_to_monitor+=("$line"); done <<< "$included_containers"
...
# ... the rest of the functions from show_container_info to the end of the file are exactly the same ...
# I will paste them all for completeness.
...
configure_exclusion_list() {
    ...
    while true; do
        ...
        while IFS= read -r line; do all_containers_array+=("$line"); done < <(docker ps --format '{{.Names}}')
        local items_str=""
        local i=0
        ...
        if [ ${#excluded_map[@]} -gt 0 ]; then current_excluded_display=$(IFS=, ; echo "${!excluded_map[*]}"); fi
        items_str="$items_str\n" 
        ...
        case "$choice" in
            c|C) break ;;
            "") excluded_map=(); log_info "已清空脚本内配置，将使用 config.json 的备用配置。"; sleep 1.5; break ;;
            *)
                ...
                for index in "${selected_indices[@]}"; do
                    if [[ "$index" =~ ^[0-9]+$ ]] && [ "$index" -ge 1 ] && [ "$index" -le ${#all_containers_array[@]} ]; then
                        local target_container="${all_containers_array[$(expr $index - 1)]}"
                        if [ -n "${excluded_map[$target_container]+_}" ]; then unset excluded_map["$target_container"]; else excluded_map["$target_container"]=1; fi
    ...
    if [ ${#excluded_map[@]} -gt 0 ]; then final_excluded_list=$(IFS=,; echo "${!excluded_map[*]}"); fi
...
configure_watchtower(){ 
    ...
    local temp_extra_args="${WATCHTOWER_EXTRA_ARGS:-}"; if echo "$extra_args_choice" | grep -qE '^[Yy]$'; then read -r -p "请输入额外参数: " temp_extra_args; fi
    ...
    _print_header "配置确认"
    local final_exclude_list; final_exclude_list="${WATCHTOWER_EXCLUDE_LIST:-${WT_EXCLUDE_CONTAINERS_FROM_CONFIG:-无}}"; local source_msg; if [ -n "${WATCHTOWER_EXCLUDE_LIST:-}" ]; then source_msg="脚本"; else source_msg="config.json"; fi
    ...
    echo -e "${COLOR_YELLOW}╰$(generate_line)╯${COLOR_RESET}"; read -r -p "确认应用此配置吗? ([y/回车]继续, [n]取消): " confirm_choice
    ...
    if ! _start_watchtower_container_logic "$WT_INTERVAL_TMP" "Watchtower模式"; then echo -e "${COLOR_RED}❌ Watchtower 启动失败。${COLOR_RESET}"; return 1; fi
    return 0
}
manage_tasks(){ while true; do if [ "${JB_ENABLE_AUTO_CLEAR}" = "true" ]; then clear; fi; local items="  1. › 停止/移除 Watchtower\n  2. › 移除 Cron\n  3. › 移除 Systemd Timer\n  4. › 重启 Watchtower"; _render_menu "⚙️ 任务管理 ⚙️" "$items"; read -r -p " └──> 请选择, 或按 Enter 返回: " choice; case "$choice" in 1) if docker ps -a --format '{{.Names}}' | grep -q '^watchtower$'; then if confirm_action "确定移除 Watchtower？"; then set +e; docker rm -f watchtower &>/dev/null; set -e; WATCHTOWER_ENABLED="false"; save_config; send_notify "🗑️ Watchtower 已从您的服务器移除。"; echo -e "${COLOR_GREEN}✅ 已移除。${COLOR_RESET}"; fi; else echo -e "${COLOR_YELLOW}ℹ️ Watchtower 未运行。${COLOR_RESET}"; fi; press_enter_to_continue ;; 4) if docker ps -a --format '{{.Names}}' | grep -q '^watchtower$'; then echo "正在重启..."; if docker restart watchtower; then send_notify "🔄 Watchtower 服务已重启。"; echo -e "${COLOR_GREEN}✅ 重启成功。${COLOR_RESET}"; else echo -e "${COLOR_RED}❌ 重启失败。${COLOR_RESET}"; fi; else echo -e "${COLOR_YELLOW}ℹ️ Watchtower 未运行。${COLOR_RESET}"; fi; press_enter_to_continue ;; *) if [ -z "$choice" ]; then return; else log_warn "无效选项"; sleep 1; fi;; esac; done; }
...
_get_watchtower_remaining_time(){
    local int="$1"; local logs="$2"; if [ -z "$int" ] || [ -z "$logs" ]; then echo -e "${YELLOW}N/A${COLOR_RESET}"; return; fi
    local log_line=""; local ts=""; local epoch=0; local rem=0
    log_line=$(echo "$logs" | grep -E "Session done|Scheduling first run|Starting Watchtower" | tail -n 1 || true)
    if [ -z "$log_line" ]; then echo -e "${YELLOW}等待首次扫描...${COLOR_RESET}"; return; fi
    ts=$(_parse_watchtower_timestamp_from_log_line "$log_line"); epoch=$(_date_to_epoch "$ts")
    if [ "$epoch" -gt 0 ]; then
        if [[ "$log_line" == *"Session done"* ]]; then rem=$(expr $int - \( $(date +%s) - $epoch \));
        elif [[ "$log_line" == *"Scheduling first run"* ]]; then rem=$(expr $epoch - $(date +%s));
        elif [[ "$log_line" == *"Starting Watchtower"* ]]; then rem=$(expr \( $epoch + 5 + $int \) - $(date +%s)); fi
        if [ "$rem" -gt 0 ]; then printf "%b%02d时%02d分%02d秒%b" "$COLOR_GREEN" $(expr $rem / 3600) $(expr \( $rem % 3600 \) / 60) $(expr $rem % 60) "$COLOR_RESET"; else printf "%b即将进行%b" "$COLOR_GREEN" "$COLOR_RESET"; fi
    else echo -e "${YELLOW}计算中...${NC}"; fi
}
...
show_watchtower_details(){ 
    while true; do
        ...
        local content_lines; content_lines="上次活动: $(get_last_session_time :- "未检测到")\n"; content_lines="${content_lines}下次检查: $countdown\n"; content_lines="${content_lines}${COLOR_BLUE}$(generate_line 60)${COLOR_RESET}\n"; content_lines="${content_lines}最近 24h 摘要：\n"
        local updates; updates=$(get_updates_last_24h || true)
        if [ -z "$updates" ]; then content_lines="${content_lines}无日志事件。"; else content_lines="${content_lines}$(echo "$updates" | tail -n 200 | while IFS= read -r line; do _format_and_highlight_log_line "$line"; done)"; fi
...
view_and_edit_config(){
    ...
    config_items=( "TG Token|TG_BOT_TOKEN|string" "TG Chat ID|TG_CHAT_ID|string" "Email|EMAIL_TO|string" "额外参数|WATCHTOWER_EXTRA_ARGS|string" "调试模式|WATCHTOWER_DEBUG_ENABLED|bool" "检查间隔|WATCHTOWER_CONFIG_INTERVAL|interval" "Watchtower 启用状态|WATCHTOWER_ENABLED|bool" "Cron 执行小时|CRON_HOUR|number_range|0-23" "Cron 项目目录|DOCKER_COMPOSE_PROJECT_DIR_CRON|string" "Cron 任务启用状态|CRON_TASK_ENABLED|bool" )
    while true; do
        if [ "${JB_ENABLE_AUTO_CLEAR}" = "true" ]; then clear; fi; load_config
        local content_lines=""; local i
        for i in "${!config_items[@]}"; do
            local item="${config_items[$i]}"; local label; label=$(echo "$item" | cut -d'|' -f1); local var_name; var_name=$(echo "$item" | cut -d'|' -f2); local type; type=$(echo "$item" | cut -d'|' -f3); local current_value="${!var_name}"; local display_text=""; local color="${COLOR_CYAN}"
            case "$type" in
                string) if [ -n "$current_value" ]; then color="${COLOR_GREEN}"; display_text="$current_value"; else color="${COLOR_RED}"; display_text="未设置"; fi ;;
                bool) if [ "$current_value" = "true" ]; then color="${COLOR_GREEN}"; display_text="是"; else color="${COLOR_CYAN}"; display_text="否"; fi ;;
                interval) display_text=$(_format_seconds_to_human "$current_value"); if [ "$display_text" != "N/A" ]; then color="${COLOR_GREEN}"; else color="${COLOR_RED}"; display_text="未设置"; fi ;;
                number_range) if [ -n "$current_value" ]; then color="${COLOR_GREEN}"; display_text="$current_value"; else color="${COLOR_RED}"; display_text="未设置"; fi ;;
            esac
            content_lines+=$(printf "\n %2d. %-20s: %b%s%b" "$(expr $i + 1)" "$label" "$color" "$display_text" "$COLOR_RESET")
        done
        _render_menu "⚙️ 配置查看与编辑 ⚙️" "$content_lines"; read -r -p " └──> 输入编号编辑, 或按 Enter 返回: " choice
        if [ -z "$choice" ]; then return; fi
        if ! echo "$choice" | grep -qE '^[0-9]+$' || [ "$choice" -lt 1 ] || [ "$choice" -gt "${#config_items[@]}" ]; then log_warn "无效选项。"; sleep 1; continue; fi
        local selected_index=$(expr $choice - 1); local selected_item="${config_items[$selected_index]}"; local label; label=$(echo "$selected_item" | cut -d'|' -f1); local var_name; var_name=$(echo "$selected_item" | cut -d'|' -f2); local type; type=$(echo "$selected_item" | cut -d'|' -f3); local extra; extra=$(echo "$selected_item" | cut -d'|' -f4); local current_value="${!var_name}"; local new_value=""
        case "$type" in
            string) read -r -p "请输入新的 '$label' (当前: $current_value): " new_value; declare "$var_name"="${new_value:-$current_value}" ;;
            bool) read -r -p "是否启用 '$label'? (y/N): " new_value; if echo "$new_value" | grep -qE '^[Yy]$'; then declare "$var_name"="true"; else declare "$var_name"="false"; fi ;;
            ...
            number_range)
                ...
                while true; do read -r -p "请输入新的 '$label' (${min}-${max}, 当前: $current_value): " new_value; if [ -z "$new_value" ]; then break; fi; if echo "$new_value" | grep -qE '^[0-9]+$' && [ "$new_value" -ge "$min" ] && [ "$new_value" -le "$max" ]; then declare "$var_name"="$new_value"; break; else log_warn "无效输入, 请输入 ${min} 到 ${max} 之间的数字。"; fi; done ;;
        esac
