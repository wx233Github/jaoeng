#!/usr/bin/env bash
#
# Docker 自动更新助手 (v3.4.0 - 最终修复日志查看与返回逻辑)
#
set -euo pipefail

export LC_ALL=C.utf8

VERSION="v3.4.0-final-log-and-return-fix"
...
# ========= FIX START: 修复日志查看和返回逻辑 =========
show_container_info() { 
    while true; do 
        if [[ "${JB_ENABLE_AUTO_CLEAR}" == "true" ]]; then clear; fi; 
        echo -e "${COLOR_YELLOW}📋 容器管理 📋${COLOR_RESET}"; 
        ...
        read -r -p "输入编号操作, 或按 Enter 返回: " choice; 
        case "$choice" in 
            "") return ;; 
            *) 
                ...
                if [[ "${JB_ENABLE_AUTO_CLEAR}" == "true" ]]; then clear; fi; 
                echo -e "${COLOR_CYAN}操作容器: ${selected_container}${COLOR_RESET}"; 
                echo "1. 日志"; echo "2. 重启"; echo "3. 停止"; echo "4. 删除"; echo; 
                read -r -p "请选择, 或按 Enter 返回: " action; 
                case "$action" in 
                    1) 
                        echo -e "${COLOR_YELLOW}日志 (Ctrl+C 停止)...${COLOR_RESET}"; 
                        trap '' INT # 临时禁用全局 Ctrl+C 陷阱
                        docker logs -f --tail 100 "$selected_container" || true # 移除 2>/dev/null
                        trap 'echo -e "\n操作被中断。"; exit 10' INT # 恢复陷阱
                    ...
                    4) if confirm_action "警告: 删除 '${selected_container}'？"; then echo "删除中..."; if docker rm -f "$selected_container"; then echo -e "${COLOR_GREEN}✅ 成功。${COLOR_RESET}"; else echo -e "${COLOR_RED}❌ 失败。${COLOR_RESET}"; fi; sleep 1; else echo "已取消。"; fi ;; 
...
view_and_edit_config(){ 
    while true; do 
        if [[ "${JB_ENABLE_AUTO_CLEAR}" == "true" ]]; then clear; fi; 
        load_config
        echo -e "${COLOR_YELLOW}⚙️ 配置查看与编辑 ⚙️${COLOR_RESET}"
        echo "---"
        local val;
        ...
        case "$choice" in 
            ...
            5) read -r -p "启用调试？(y/n): " d; WATCHTOWER_DEBUG_ENABLED=$([ "$d" =~ ^[Yy]$ ] && echo "true" || echo "false"); save_config ;; 
            ...
            7) read -r -p "启用 Watchtower？(y/n): " d; WATCHTOWER_ENABLED=$([ "$d" =~ ^[Yy]$ ] && echo "true" || echo "false"); save_config ;; 
            8) while true; do read -r -p "新 Cron 小时(0-23): " a; if [ -z "$a" ]; then break; fi; if [[ "$a" =~ ^[0-9]+$ ]] && [ "$a" -ge 0 ] && [ "$a" -le 23 ]; then CRON_HOUR="$a"; save_config; break; else echo "无效"; fi; done ;; 
            9) read -r -p "新 Cron 目录: " a; DOCKER_COMPOSE_PROJECT_DIR_CRON="${a:-$DOCKER_COMPOSE_PROJECT_DIR_CRON}"; save_config ;; 
            10) read -r -p "启用 Cron？(y/n): " d; CRON_TASK_ENABLED=$([ "$d" =~ ^[Yy]$ ] && echo "true" || echo "false"); save_config ;; 
            "") return ;; 
