#!/bin/bash
# =============================================================
# 🚀 Docker 自动更新助手 (v3.9.6 - Refactored & Bug Fixed)
# =============================================================
...
_start_watchtower_container_logic(){
  ...
  local containers_to_monitor=(); if [ -n "$final_exclude_list" ]; then
      ...
      if [ -n "$included_containers" ]; then
          # Using a while read loop for max compatibility instead of readarray
            ...
            *)
                ...
                for index in "${selected_indices[@]}"; do
                    ...
                    if echo "$index" | grep -qE '^[0-9]+$'; then
                        if [ "$index" -ge 1 ] && [ "$index" -le ${#all_containers_array[@]} ]; then
                            is_valid_index=true
                        fi
                    fi
...
# =============================================================
# START: Refactored view_and_edit_config function
# =============================================================
view_and_edit_config(){
    # Data-driven configuration definition
    # Format: "Menu Label|VARIABLE_NAME|TYPE|Optional:Extra Param"
    # TYPE can be: string, bool, interval, number_range
    ...
    while true; do
            ...
            case "$type" in
                string)
                    if [ -n "$current_value" ]; then
                        color="${COLOR_GREEN}"; display_text="$current_value"
                    else
                        color="${COLOR_RED}"; display_text="未设置"
                    fi
                    ;;
                bool)
                    if [ "$current_value" = "true" ]; then
                        color="${COLOR_GREEN}"; display_text="是"
                    else
                        color="${COLOR_CYAN}"; display_text="否"
                    fi
                    ;;
                interval)
                    display_text=$(_format_seconds_to_human "$current_value")
                    if [ "$display_text" != "N/A" ]; then
                        color="${COLOR_GREEN}"
                    else
                        color="${COLOR_RED}"; display_text="未设置"
                    fi
        ...
        _render_menu "⚙️ 配置查看与编辑 ⚙️" "$content_lines"
        ...
        local selected_index=$(expr $choice - 1)
        ...
        case "$type" in
            string)
                ...
                # If user enters nothing, keep the old value
                if [ -z "$new_value" ] && [[ "$#" -eq 0 ]]; then
                    declare "$var_name"="$current_value"
                else
            ...
            bool)
                ...
                if echo "$new_value" | grep -qE '^[Yy]$'; then
...
# =============================================================
# END: Refactored view_and_edit_config function
# =============================================================
...
main_menu(){
  while true; do
    ...
    # Safely build an array of lines
    ...
    # Use printf to safely pass multiline content to the renderer
