#!/bin/bash
# 🚀 Docker 自动更新助手
# v2.12.0 进一步优化：持久化Watchtower/Cron启用状态，统一Watchtower启动逻辑，增强配置编辑提示，更健壮的Watchtower参数获取。
# 功能：
...
show_status() {
    ...
    echo "-------------------------------------------------------------------------------------------------------------------"
    # 这里不加 press_enter_to_continue，因为此函数在脚本启动时就调用，避免额外暂停
}
