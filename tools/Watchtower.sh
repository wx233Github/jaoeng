...
# --- 辅助函数 & 日志系统 ---
sudo_preserve_env() { sudo -E "$@"; } # 保留以防万一，但核心逻辑不再依赖
setup_logging() {
    export LC_ALL=C.utf8
...
display_menu() {
    ...
    local config_path="${CONFIG[install_dir]}/config.json"; local header_text="🚀 VPS 一键安装入口 (v9.9)"; if [ "$CURRENT_MENU_NAME" != "MAIN_MENU" ]; then header_text="🛠️ ${CURRENT_MENU_NAME//_/ }"; fi
...
main() {
    export LC_ALL=C.utf8
    # ... (与 v9.8 main 函数相同, 仅为完整性粘贴) ...
    local CACHE_BUSTER=""; declare -A CONFIG; CONFIG[install_dir]="/opt/vps_install_modules"; CONFIG[lock_file]="/tmp/vps_install_modules.lock"; CONFIG[base_url]="https://raw.githubusercontent.com/wx233Github/jaoeng/main"; if [[ "${ONLINE_INSTALL}" == "true" ]]; then CACHE_BUSTER="?_=$(date +%s)"; echo -e "${YELLOW}[警告]${NC} 在线安装模式：将强制拉取所有最新文件。"; sudo rm -f "${CONFIG[install_dir]}/config.json" 2>/dev/null || true; fi; acquire_lock; trap 'release_lock; log_info "脚本已退出。"' EXIT HUP INT QUIT TERM; sudo mkdir -p "${CONFIG[install_dir]}"; local config_path="${CONFIG[install_dir]}/config.json"; if [ ! -f "$config_path" ]; then echo -e "${BLUE}[信息]${NC} 未找到配置，正在下载..."; if ! curl -fsSL "${CONFIG[base_url]}/config.json${CACHE_BUSTER}" -o "$config_path"; then echo -e "${RED}[错误]${NC} 下载失败！"; exit 1; fi; echo -e "${GREEN}[成功]${NC} 已下载。"; fi; CONFIG[dependencies]='curl cmp ln dirname flock jq'; CONFIG[enable_auto_clear]="false"; CONFIG[timezone]="Asia/Shanghai"; if ! command -v jq &>/dev/null; then check_and_install_dependencies; fi; load_config; setup_logging; log_info "脚本启动 (v9.9)"; check_and_install_dependencies; local SCRIPT_PATH="${CONFIG[install_dir]}/install.sh"; if [ ! -f "$SCRIPT_PATH" ]; then save_entry_script; fi; setup_shortcut; self_update; while true; do display_menu; local exit_code=0; process_menu_selection || exit_code=$?; if [ "$exit_code" -ne 10 ]; then while read -r -t 0; do :; done; read -p "$(echo -e "${BLUE}按回车键继续...${NC}")"; fi; done
}
main "$@"
