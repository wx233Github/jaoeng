#!/bin/bash
# =============================================================
# 🚀 tcp_optimizer.sh (v6.3.0 - 生产级容灾版 / 全栈极限重构)
# =============================================================
# 作者：System Admin
# 描述：全景 Linux 网络调优引擎。引入全自动备份恢复，彻底根除环境检测聚合错误。
# 版本历史：
#   v6.3.0 - 新增备份恢复系统，重构环境检测引擎根除'nonenone'，修复虚拟化回退逻辑覆盖
#   v6.2.1 - 修复 check_root 在 root 用户下因 set -e 崩溃的致命逻辑漏洞
#   v6.2.0 - 修复缺少 Curl 时的启动死锁，优化 rng-tools 容错
# =============================================================
...
check_dependencies() {
    local deps=(sysctl uname sed modprobe grep awk ip ping timeout ethtool bc curl wget gpg ss rngd tar)
    local missing=()
    local install_list=()
    for cmd in "${deps[@]}"; do 
    ...
    if [[ ${#missing[@]} -gt 0 ]]; then
        ...
        if [[ "${ui_dep,,}" == "y" ]]; then 
            install_dependencies "${install_list[@]}" || log_warn "部分依赖安装失败..."
        else 
            exit 1
        fi
    fi
...
check_environment() {
    log_step "全景环境诊断..."
    local raw_virt=""
    local virt_type="physical" # 默认物理机
    ...
    # 彻底重构检测引擎
    raw_virt=$(echo "${raw_virt}" | tr -d '[:space:]')
    if [[ -z "${raw_virt}" || "${raw_virt}" == "none" ]]; then
        if grep -qE 'docker|lxc' /proc/1/cgroup 2>/dev/null; then
            ...
            # 保留 KVM/VMware 等为 "physical" (因为它们支持底层调优)
            virt_type="physical/kvm"
        fi
    else
        virt_type="${raw_virt}"
    fi
...
# ... [version_ge, apply_system_limits, install_xanmod_kernel, optimize_nic_hardware, inject_kernel_modules, generate_sysctl_content, manage_ipv4_precedence 等函数保持 v6.2.1 的健壮版本，此处省略以聚焦核心变更] ...
# 为了完整性，粘贴所有函数
...
generate_sysctl_content() {
    ...
    echo "# ============================================================="
    echo "# TCP Optimizer Configuration (Auto-generated v6.3.0)"
    echo "# ============================================================="
    cat <<EOF
...
restore_configs() {
    ...
    local backups=($(find "${BACKUP_DIR}" -name "*.tar.gz" | sort -r))
    ...
    select backup_choice in "${backups[@]}"; do
        if [[ -n "${backup_choice}" ]]; then
            log_step "正在从 ${backup_choice} 恢复..."
            # 先清理当前所有配置
            rm -f "${CONFIG_FILES[@]}"
            tar -xzf "${backup_choice}" -C /
...
show_menu() {
    ...
    echo "========================================================"
    echo -e " 🚀 终极画像调优引擎 ${COLOR_YELLOW}(v6.3.0 Disaster Recovery Edition)${COLOR_RESET}"
    echo "========================================================"
