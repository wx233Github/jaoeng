#!/bin/bash
# =============================================================
# 🚀 tcp_optimizer.sh (v6.5.0 - 绝对健壮 & 自我审计版)
# =============================================================
# 作者：System Admin
# 描述：全景 Linux 网络调优引擎。彻底修复 here-doc 语法，引入配置审计工具。
# 版本历史：
#   v6.5.0 - 绝对修复 here-doc 语法，新增配置审计工具，重组菜单
#   v6.4.1 - 修复 here-document 语法崩溃，移除终端时间戳，增强卸载原子性
...
log_msg() { local level="$1" color="$2" msg="$3"; printf "${color}[%s] %s${COLOR_RESET}\n" "${level}" "${msg}" >&2; echo "[$(date '+%F %T')] [${level}] ${msg}" >> "${LOG_FILE}"; }
...
error_handler() {
    local exit_code=$? line_no=$1 command="${BASH_COMMAND}"
    if [[ $exit_code -ne 0 ]]; then log_error "脚本异常退出! (Line: ${line_no}, Command: '${command}', ExitCode: ${exit_code})"; fi
}
trap 'error_handler ${LINENO}' EXIT

# ... [check_* functions and other mature functions are unchanged] ...
...
EOF
    if [[ ${IS_SYSTEMD} -eq 1 ]]; then
        ...
        cat > "${SYSTEMD_SYS_CONF}" <<-EOF
			[Manager]
			DefaultLimitNOFILE=1048576
EOF
        cat > "${SYSTEMD_USR_CONF}" <<-EOF
			[Manager]
			DefaultLimitNOFILE=1048576
EOF
...
EOF
    systemctl daemon-reload && systemctl enable --now nic-optimize.service 2>/dev/null || true; fi; }
...
remove_old_kernels() {
    ...
    if ! command -v dpkg &>/dev/null; then log_warn "非 Debian/Ubuntu 系统，暂不支持内核自动清理。"; return; fi
    local current_kernel; current_kernel=$(uname -r); local kernels_to_remove=(); 
    # shellcheck disable=SC2207
    kernels_to_remove=($(dpkg --list | grep 'linux-image' | awk '{ print $2 }' | grep -v "${current_kernel}"))
    if [[ ${#kernels_to_remove[@]} -eq 0 ]]; then log_info "没有发现可清理的旧内核。"; return; fi
    echo "以下旧内核将被清理:"; printf " - %s\n" "${kernels_to_remove[@]}"; read -rp "确认要继续吗? [y/N]: " confirm
    if [[ "${confirm,,}" != "y" ]]; then log_warn "操作已取消。"; return; fi
    export DEBIAN_FRONTEND=noninteractive; apt-get purge -y "${kernels_to_remove[@]}"; apt-get autoremove -y; update-grub 2>/dev/null || true; log_info "旧内核清理完成。"
}

kernel_manager() {
    echo "--- 内核维护工具 ---"; echo "1. 安装/更新 XanMod 内核"; echo "2. 清理所有冗余旧内核"; echo "0. 返回主菜单"; read -rp "请选择操作 [0-2]: " choice
    case "${choice}" in 1) install_xanmod_kernel ;; 2) remove_old_kernels ;; 0|*) return ;; esac
}

install_xanmod_kernel() {
    if [[ ${IS_CONTAINER} -eq 1 ]]; then log_warn "容器环境无法更换内核。"; return; fi
    echo -e "${COLOR_BLUE}========================================================${COLOR_RESET}"; echo -e "${COLOR_BLUE}   XanMod Kernel 安装向导 (Debian/Ubuntu Only)          ${COLOR_RESET}"; echo -e "${COLOR_BLUE}========================================================${COLOR_RESET}";
    ...
    export DEBIAN_FRONTEND=noninteractive; local DPKG_OPTS="-o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confold"; log_step "正在导入 XanMod GPG Key...";
    wget -qO - https://dl.xanmod.org/archive.key | gpg --dearmor -o /usr/share/keyrings/xanmod-archive-keyring.gpg --yes
    echo 'deb [signed-by=/usr/share/keyrings/xanmod-archive-keyring.gpg] http://deb.xanmod.org releases main' | tee /etc/apt/sources.list.d/xanmod-release.list
    apt-get update -y; log_step "安装 linux-xanmod-x64v3 (极端静默模式)..."
    # shellcheck disable=SC2086
    if apt-get install -yq ${DPKG_OPTS} linux-xanmod-x64v3; then echo -e "${COLOR_GREEN}XanMod 内核安装成功！请在脚本结束后重启服务器以生效。${COLOR_RESET}"; else log_error "安装失败，请检查网络。"; fi
}

manage_backups() {
    # shellcheck disable=SC2207
    local backups=($(ls -t "${BACKUP_DIR}"/config_backup_*.tar.gz 2>/dev/null || true))
    if [[ ${#backups[@]} -gt ${MAX_BACKUPS} ]]; then
        log_info "备份数量超出限制(${MAX_BACKUPS})，正在清理最旧的备份..."
        ls -tr "${BACKUP_DIR}"/config_backup_*.tar.gz | head -n $((${#backups[@]} - MAX_BACKUPS)) | xargs -r rm -f
    fi
}
...
backup_configs() {
    ...
    local files_to_backup=(); for f in "${CONFIG_FILES[@]}"; do if [[ -f "${f}" ]]; then files_to_backup+=("${f}"); fi; done
...
restore_configs() {
    log_step "正在查找可用备份..."; local backups=();
    # shellcheck disable=SC2207
    backups=($(find "${BACKUP_DIR}" -name "*.tar.gz" 2>/dev/null | sort -r))
    ...
    select backup_choice in "${backups[@]}"; do
        ...
        if tar -xzf "${backup_choice}" -C "${temp_dir}"; then
            log_info "备份文件验证通过。正在应用..."; rm -f "${CONFIG_FILES[@]}"; cp -r "${temp_dir}"/* /
            if [[ ${IS_SYSTEMD} -eq 1 ]]; then systemctl daemon-reload; systemctl restart systemd-sysctl; systemctl enable --now nic-optimize.service 2>/dev/null || true; fi
            log_info "配置恢复并已应用。"
            return 0
        else
            log_error "备份文件已损坏或解压失败！当前配置未受影响。"; return 1
        fi
    done
}
...
audit_configs() {
    ...
    while IFS= read -r line; do
        ...
        current_val=$(echo "$current_val" | tr -d '[:space:]' | tr '\t' ' ') # 标准化内核输出

        if [[ "${current_val}" == "${val}" ]]; then
            printf "${COLOR_GREEN}[MATCH]${COLOR_RESET} %-45s = %s\n" "${key}" "${val}"
        else
            printf "${COLOR_YELLOW}[MISMATCH]${COLOR_RESET} %-45s | Expected: %s | Current: %s\n" "${key}" "${val}" "${current_val}"
...
show_menu() {
    ...
    echo "========================================================"
    echo -e " 🚀 终极画像调优引擎 ${COLOR_YELLOW}(v6.5.0 Audit Edition)${COLOR_RESET}"
    echo "========================================================"
    ...
    echo -e " 7. ${COLOR_YELLOW}从备份恢复上次配置 (时光机)${COLOR_RESET}"
    echo -e " 8. ${COLOR_CYAN}审计当前系统配置 (检查生效状态)${COLOR_RESET}"
    echo " 9. 彻底卸载/恢复系统默认"
    echo "--------------------------------------------------------"
    echo " 0. 退出"
    echo "========================================================"
...
main() {
    ...
    while true; do
        ...
        case "$c" in
            ...
            7) restore_configs; read -rp "按回车继续...";;
            8) audit_configs; read -rp "按回车继续...";;
            9) 
                ...
                if [[ ${IS_SYSTEMD} -eq 1 ]]; then 
                    ...
                    # 写入空的 drop-in 来覆盖可能存在的手动配置，确保恢复默认
