#!/bin/bash
# =============================================================
# 🚀 tcp_optimizer.sh (v6.4.1 - 语法归正 & UX 优化版)
# =============================================================
# 作者：System Admin
# 描述：全景 Linux 网络调优引擎。修复 here-doc 语法，优化日志输出，增强卸载原子性。
# 版本历史：
...
log_msg() {
    ...
    local terminal_msg="[${level}] ${msg}"
    local log_msg="[$(date '+%F %T')] [${level}] ${msg}"
...
# ... [check_* functions, version_ge, etc. from v6.3.0 are mature and unchanged] ...
...
restore_configs() {
    ...
    select backup_choice in "${backups[@]}"; do
        ...
        if tar -xzf "${backup_choice}" -C "${temp_dir}"; then
            ...
            if [[ ${IS_SYSTEMD} -eq 1 ]]; then systemctl daemon-reload; systemctl enable --now nic-optimize.service 2>/dev/null || true; fi
            sysctl --system >/dev/null 2>&1; log_info "配置恢复并已应用。"
            return 0
        else
...
show_menu() {
    ...
    echo "========================================================"
    echo -e " 🚀 终极画像调优引擎 ${COLOR_YELLOW}(v6.4.1 Kernel Manager Edition)${COLOR_RESET}"
    echo "========================================================"
