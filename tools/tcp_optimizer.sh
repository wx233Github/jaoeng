...
install_xanmod_kernel() { if [[ ${IS_CONTAINER} -eq 1 ]]; then log_warn "容器环境无法更换内核。"; return; fi; echo -e "${COLOR_BLUE}========================================================${COLOR_RESET}"; echo -e "${COLOR_BLUE}   XanMod Kernel 安装向导 (Debian/Ubuntu Only)          ${COLOR_RESET}"; echo -e "${COLOR_BLUE}========================================================${COLOR_RESET}"; if grep -iq "xanmod" /proc/version 2>/dev/null; then log_info "✅ 检测到当前已运行 XanMod 内核。"; read -rp "按回车继续..."; return; fi; if [[ ! -f /etc/debian_version ]]; then log_warn "非 Debian/Ubuntu，暂不支持自动安装 XanMod。"; return; fi; read -rp "是否需要先清理旧内核为新内核腾出空间? [y/N]: " confirm_clean; if [[ "${confirm_clean,,}" == "y" ]]; then remove_old_kernels; fi; read -rp "是否继续安装 XanMod Kernel (推荐 x64v3)? [y/N]: " ui_inst; if [[ "${ui_inst,,}" != "y" ]]; then return; fi; export DEBIAN_FRONTEND=noninteractive; local DPKG_OPTS="-o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confold"; log_step "正在导入 XanMod GPG Key..."; wget -qO - https://dl.xanmod.org/archive.key | gpg --dearmor -o /usr/share/keyrings/xanmod-archive-keyring.gpg --yes; echo 'deb [signed-by=/usr/share/keyrings/xanmod-archive-keyring.gpg] http://deb.xanmod.org releases main' | tee /etc/apt/sources.list.d/xanmod-release.list; apt-get update -y; log_step "安装 linux-xanmod-x64v3 (极端静默模式)..."; if apt-get install -yq ${DPKG_OPTS} linux-xanmod-x64v3; then echo -e "${COLOR_GREEN}XanMod 内核安装成功！请在脚本结束后重启服务器以生效。${COLOR_RESET}"; else log_error "安装失败，请检查网络。"; fi; }
    ...
    while true; do
        ...
        case "$c" in
            ...
            9) 
                ...
                read -rp "确认要彻底卸载吗? [y/N]: " confirm_uninstall
