...
log_message() {    
    ...
    else # 非交互模式，只输出 INFO/WARN/ERROR，并带上级别前缀，方便cron日志分析
        if [ "$level" = "INFO" ] || [ "$level" = "WARN" ] || [ "$level" = "ERROR" ]; then
            echo "[${timestamp}] [${level}] ${message}"
        fi
        if [ "$level" = "DEBUG" ]; then # DEBUG 级别只记录文件，不输出到非交互模式的终端
             true # No-op
        fi
    fi
    ...
    for cmd in "${!DEPS_MAP[@]}"; do    
        ...
        if command -v "$cmd" &>/dev/null; then    
            ...
            if [ "$INSTALLED_VER" != "not-found" ] && [ "$INSTALLED_VER" = "$AVAILABLE_VER" ]; then    
                ...
                log_message DEBUG "命令 '$cmd' (由包 '$pkg') 正在安装或更新至最新版 ($INSTALLED_VER -> $AVAILABLE_VER)..." # 记录日志  
                # 将安装过程的输出重定向到日志文件  
            ...
            log_message DEBUG "缺少命令 '$cmd' (由包 '$pkg' 提供)，正在安装..." # 记录日志  
            # 将安装过程的输出重定向到日志文件  
...
configure_nginx_projects() {    
    ...
    if [ "$ACME_CA_SERVER_NAME" = "zerossl" ]; then    
        ...
        if ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then    
             ...
             if ! "$ACME_BIN" --register-account -m "$ZERO_SSL_ACCOUNT_EMAIL" --server "$ACME_CA_SERVER_URL"; then    
    ...
    for P in "${PROJECTS[@]}"; do    
        ...
        if [ "$SHOULD_ISSUE_CERT" = "y" ]; then    
            ...
            local INSTALL_CERT_COMMAND_ARRAY=("$ACME_BIN" "--install-cert" "-d" "$MAIN_DOMAIN" "--ecc" \    
                "--key-file" "$INSTALLED_KEY_FILE" \    
                "--fullchain-file" "$INSTALLED_CRT_FILE" \    
                "--reloadcmd" "systemctl reload nginx")    
            if [ "$USE_WILDCARD" = "y" ]; then    
                INSTALL_CERT_COMMAND_ARRAY+=("-d" "*.$MAIN_DOMAIN") # Wildcard should be literal `*.domain`    
            fi    
    
            if ! "${INSTALL_CERT_COMMAND_ARRAY[@]}" ; then log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; continue; fi    
        else    
    ...
    while true; do    
        ...
        case "$MANAGE_CHOICE" in    
            ...
            3) # 编辑项目核心配置 (不含片段)    
                ...
                if [[ "$EDIT_ACME_VALIDATION_METHOD" = "imported" || "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]]; then    
                    ...
                    if [ "$NEW_CA_SERVER_NAME" = "zerossl" ]; then    
                         ...
                         if ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then    
                            ...
                            if ! "$ACME_BIN" --register-account -m "$NEW_ZERO_SSL_ACCOUNT_EMAIL" --server "$NEW_CA_SERVER_URL"; then    
                                log_message ERROR "❌ ZeroSSL 账户注册失败！请检查邮箱地址或稍后重试。"    
                                sleep 2    
                                continue # 返回编辑菜单  
                            fi    
                ...
                if [ "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]; then    
                    ...
                    if [[ "$UPDATE_NOW" =~ ^[Yy]$ ]]; then    
                        ...
                        local INSTALL_CERT_COMMAND_ARRAY=("$ACME_BIN" "--install-cert" "-d" "$DOMAIN_TO_EDIT" "--ecc" \    
                            "--key-file" "$NEW_KEY_FILE" \    
                            "--fullchain-file" "$NEW_CERT_FILE" \    
                            "--reloadcmd" "systemctl reload nginx")    
                        if [ "$NEW_USE_WILDCARD" = "y" ]; then    
                            INSTALL_CERT_COMMAND_ARRAY+=("-d" "*.$DOMAIN_TO_EDIT") # Wildcard should be literal `*.domain`    
                        fi    
                        if ! "${INSTALL_CERT_COMMAND_ARRAY[@]}" ; then log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; sleep 2; continue; fi    
            ...
            4) # 管理自定义 Nginx 配置片段    
                ...
                # 这段逻辑应在内部 while 循环结束后，并且在整个 case 4) 的 ;; 之前  
                # 如果从内层循环通过 `break 2` 退出了整个 manage_configs 循环，则这部分不会执行  
                # 只有当内层循环通过 `break` 退出到这里时，才会执行  
                if [[ "$SNIPPET_MANAGEMENT_ACTION" != "0" ]]; then # 如果不是选择 0 返回上级菜单，就执行更新逻辑  
                ...
                fi # End of if-block for snippet management action  
                sleep 2    
                ;;    
