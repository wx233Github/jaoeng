...
_log_prefix() {
    if [ "${JBLOG_WITH_TIMESTAMP:-false}" = "true" ]; then echo -n "$(date '+%Y-%m-%d %H:%M:%S') "; fi
}
...
log_message() {
    ...
    case "$level" in
        ...
        WARN)    echo -e "$(_log_prefix)${YELLOW}⚠️  [警告]${NC} $message" ;;
...
press_enter_to_continue() { read -r -p "$(echo -e "${YELLOW}⌨️  按 Enter 键继续...${NC}")" < /dev/tty; }

_prompt_for_menu_choice_local() {
    local range="$1"
    local allow_empty="${2:-false}"
    local prompt_text="${ORANGE}👉 选 项 [${range}]${NC} (↩ 返 回 ): "
    local choice
    while true; do
        read -r -p "$(echo -e "$prompt_text")" choice < /dev/tty
        if [ -z "$choice" ]; then
            if [ "$allow_empty" = "true" ]; then echo ""; return; fi
            echo -e "${YELLOW}⚠️  请选择一个选项。${NC}"
        elif [[ ! "$choice" =~ ^[0-9]+$ ]]; then
            echo -e "${YELLOW}⚠️  请输入数字。${NC}"
        else
            echo "$choice"; return;
        fi
    done
...
_render_menu() {
    local title="$1"; shift; local max_width=42
...
_prompt_user_input_with_validation() {
    local prompt="$1" default="$2" regex="$3" error_msg="$4" allow_empty="${5:-false}"
    val=""
    while true; do
        if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
            local disp=""
            if [ -n "$default" ]; then disp=" [默认: $default]"; fi
...
install_dependencies() {
    ...
    for pkg in $deps; do
        if ! command -v "$pkg" &>/dev/null && ! dpkg -s "$pkg" &>/dev/null; then
            log_message WARN "缺失: $pkg，正在安装..."
            apt-get update -y >/dev/null 2>&1 && apt-get install -y "$pkg" >/dev/null 2>&1 || missing=1
        fi
    done
    [ "$missing" -gt 0 ] && { log_message ERROR "依赖安装失败，请手动检查网络。"; return 1; }
...
_view_access_log() {
    local domain="$1"
    echo ""
    _render_menu "查看日志: $domain" "1. 访问日志 (AccessLog)" "2. 错误日志 (ErrorLog)"
    local c
    c=$(_prompt_for_menu_choice_local "1-2")
    local log_path=""
    case "$c" in
...
_issue_and_install_certificate() {
    ...
    local provider; provider=$(echo "$json" | jq -r .dns_api_provider)
    local wildcard; wildcard=$(echo "$json" | jq -r .use_wildcard)
    ...
    if [ "$method" = "dns-01" ]; then
        if [ "$provider" = "dns_cf" ]; then
            if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
                ...
                local def_t; def_t=$(grep "^SAVED_CF_Token=" "$HOME/.acme.sh/account.conf" 2>/dev/null | cut -d= -f2- | tr -d "'\"")
                local t; t=$(_prompt_user_input_with_validation "CF_Token" "$def_t" "" "不能为空" "false")
                local def_a; def_a=$(grep "^SAVED_CF_Account_ID=" "$HOME/.acme.sh/account.conf" 2>/dev/null | cut -d= -f2- | tr -d "'\"")
                local a; a=$(_prompt_user_input_with_validation "Account_ID" "$def_a" "" "不能为空" "false")
                export CF_Token="$t" CF_Account_ID="$a"
            fi
        elif [ "$provider" = "dns_ali" ]; then
            if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
                log_message INFO "🔐 请输入 Aliyun Key (仅内存暂存)"
                local def_k; def_k=$(grep "^SAVED_Ali_Key=" "$HOME/.acme.sh/account.conf" 2>/dev/null | cut -d= -f2- | tr -d "'\"")
                local k; k=$(_prompt_user_input_with_validation "Ali_Key" "$def_k" "" "不能为空" "false")
                local def_s; def_s=$(grep "^SAVED_Ali_Secret=" "$HOME/.acme.sh/account.conf" 2>/dev/null | cut -d= -f2- | tr -d "'\"")
                local s; s=$(_prompt_user_input_with_validation "Ali_Secret" "$def_s" "" "不能为空" "false")
    ...
    local log_temp; log_temp=$(mktemp)
    if ! "${cmd[@]}" > "$log_temp" 2>&1; then
        log_message ERROR "申请失败: $domain"
        cat "$log_temp"
        local err_log; err_log=$(cat "$log_temp")
        rm -f "$log_temp"
...
_gather_project_details() {
    ...
    if [ "$is_cert_only" == "false" ]; then
        ...
        while true; do
            local target; target=$(_prompt_user_input_with_validation "🔌 后端目标 (容器名/端口)" "$name" "" "" "false") || { exec 1>&3; return 1; }
    ...
    if [ "$skip_cert" == "true" ]; then
        ...
        if [[ "$ca_name" == "zerossl" ]] && ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then
             ...
             local reg_email; reg_email=$(_prompt_user_input_with_validation "注册邮箱" "" "" "" "false")
...
_display_projects_list() {
    ...
    echo "$json" | jq -c '.[]' | while read -r p; do
        ...
        if [[ -f "$cert" ]]; then
            local end; end=$(openssl x509 -enddate -noout -in "$cert" 2>/dev/null | cut -d= -f2)
            local ts; ts=$(date -d "$end" +%s 2>/dev/null || echo 0)
...
check_and_auto_renew_certs() {
    ...
    jq -c '.[]' "$PROJECTS_METADATA_FILE" | while read -r p; do
        local d; d=$(echo "$p" | jq -r .domain)
        local f; f=$(echo "$p" | jq -r .cert_file)
...
main_menu() {
    while true; do
        local nginx_status; nginx_status="$(_get_nginx_status)"
