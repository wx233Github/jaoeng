# =============================================================
# 🚀 Nginx 反向代理 + HTTPS 证书管理助手 (v4.29.0 - 全功能整合与TG通知)
# =============================================================
# 作者：Shell 脚本专家
...
_issue_and_install_certificate() {
    ...
    if [ "$method" == "reuse" ]; then log_message SUCCESS "复用关联记录，跳过申请流程。"; return 0; fi
        ...
        if [ "$provider" = "dns_cf" ]; then
            ...
            if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
                ...
                if [[ -n "$saved_k" && -n "$saved_s" ]]; then
                    if _confirm_action_or_exit_non_interactive "是否复用已保存的 Aliyun 凭证？"; then use_saved="true"; fi
                fi
                if [ "$use_saved" = "false" ]; then
...
_gather_project_details() {
    ...
    if [ "$is_cert_only" == "false" ]; then
        ...
        if [ "$skip_cert" == "false" ]; then
            ...
            local -a hook_opts=(
                "1. S-UI / 3x-ui / x-ui (执行: systemctl restart x-ui)" 
                "2. V2Ray 原生服务 (执行: systemctl restart v2ray)" 
                "3. Xray 原生服务 (执行: systemctl restart xray)" 
                "4. Nginx 服务 (执行: systemctl reload nginx)" 
                "5. 手动输入自定义 Shell 命令 (如: docker restart s-ui)" 
                "6. 跳过 (不绑定重载)"
            )
