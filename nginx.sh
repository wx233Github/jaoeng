...
# --- 强制设置 UTF-8 Locale 以正确显示中文和对齐表格 ---
# 如果您的系统没有 en_US.UTF-8 locale，请使用 'sudo locale-gen en_US.UTF-8' 生成
# 或尝试 'export LC_ALL=zh_CN.UTF-8' 如果您的系统已配置中文 locale
    ...
    for P in "${PROJECTS[@]}"; do
        ...
        case $VALIDATION_CHOICE in
            ...
            2)
                ...
                read -rp "${CYYAN}请输入序号: ${RESET}" DNS_PROVIDER_CHOICE
        ...
        if [ "$SHOULD_ISSUE_CERT" = "y" ]; then
            ...
            local INSTALL_CERT_DOMAINS_ARG=""
            INSTALL_CERT_DOMAINS_ARG+="-d \"$MAIN_DOMAIN\""
            if [ "$USE_WILDCARD" = "y" ]; then
                INSTALL_CERT_DOMAINS_ARG+=" -d \"*.$MAIN_DOMAIN\"" # Wildcard should be literal `*.domain`
            fi

            # FIX: Reconstruct the install command string correctly for eval
            local install_cert_command_str="$ACME_BIN --install-cert $INSTALL_CERT_DOMAINS_ARG --ecc"
            install_cert_command_str+=" --key-file \"$INSTALLED_KEY_FILE\""
            install_cert_command_str+=" --fullchain-file \"$INSTALLED_CRT_FILE\""
            install_cert_command_str+=" --reloadcmd \"systemctl reload nginx\""

            # acme.sh 会自动执行 --reloadcmd
            eval "$install_cert_command_str" || { log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; continue; }
        else
...
manage_configs() {
    ...
    # 注意：这里的宽度设置需要根据实际字符宽度调整，如果locale设置正确，printf会处理好UTF-8宽度
    ...
    while true; do
        ...
        case "$MANAGE_CHOICE" in
            1) # 手动续期
                ...
                if [ -z "$RENEW_PROJECT_JSON" ]; then log_message RED "❌ 域名 $DOMAIN_TO_RENEW 未找到在已配置列表中。"; sleep 1; continue; }
            ...
            3) # 编辑项目核心配置 (不含片段)
                ...
                if [ "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]; then
                    ...
                    if [[ "$UPDATE_NOW" =~ ^[Yy]$ ]]; then
                        ...
                        # FIX: Ensure INSTALL_CERT_DOMAINS is passed correctly
                        local INSTALL_CERT_DOMAINS_ARG_EDIT=""
                        INSTALL_CERT_DOMAINS_ARG_EDIT+="-d \"$DOMAIN_TO_EDIT\""
                        if [ "$NEW_USE_WILDCARD" = "y" ]; then
                            INSTALL_CERT_DOMAINS_ARG_EDIT+=" -d \"*.$DOMAIN_TO_EDIT\"" # Wildcard should be literal `*.domain`
                        fi
                        # FIX: Reconstruct the install command string correctly for eval
                        local install_cert_command_str_edit="$ACME_BIN --install-cert $INSTALL_CERT_DOMAINS_ARG_EDIT --ecc"
                        install_cert_command_str_edit+=" --key-file \"$NEW_KEY_FILE\""
                        install_cert_command_str_edit+=" --fullchain-file \"$NEW_CERT_FILE\""
                        install_cert_command_str_edit+=" --reloadcmd \"systemctl reload nginx\""

                        # acme.sh 会自动执行 --reloadcmd
                        eval "$install_cert_command_str_edit" || { log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; sleep 2; continue; }
