# =============================================================
# 🚀 Nginx 反向代理 + HTTPS 证书管理助手 (v4.24.0 - 负载均衡与压缩调优版)
# =============================================================
# 作者：Shell 脚本专家
# 描述：自动化管理 Nginx 反代配置与 SSL 证书，支持 TCP 负载均衡、TLS卸载与泛域名智能复用
# 版本历史：
#   v4.24.0 - 实装 TCP Stream 多节点负载均衡，自动注入全局 Gzip 深度压缩优化
#   v4.23.0 - 实装泛域名智能复用、独立续期参数修改，补全 TCP 快照轮转机制
#   v4.22.0 - 实装 TCP over TLS 卸载功能，彻底修复 Ctrl+C 导致的信号逃逸 Bug
...
_on_int() {
    echo -e "\n${RED}检测到中断信号 (Ctrl+C)，已安全取消操作并退出脚本。${NC}"
    _cleanup
    exit 130
}
...
_update_cloudflare_ips() {
    ...
    if curl -sS --connect-timeout 10 --max-time 15 https://www.cloudflare.com/ips-v4 > "$temp_allow" && \
       ...
       curl -sS --connect-timeout 10 --max-time 15 https://www.cloudflare.com/ips-v6 >> "$temp_allow"; then
        ...
        log_message SUCCESS "Cloudflare IP 列表更新完成。"; control_nginx reload || true
    else
...
_handle_backup_restore() {
    echo ""
    _render_menu "维护选项" "1. 备份与恢复面板 (创建全站打包备份，或回滚元数据)" "2. 重建所有 HTTP 配置 (基于元数据重写 /etc/nginx/sites-available)" "3. 修复定时任务 (重新挂载自动续期 Cron)"
                ...
                3)
                    ...
                    if [ -n "$snap_path" ] && [ -f "$snap_path" ]; then
                        if _confirm_action_or_exit_non_interactive "这将会回滚记录，确认执行？"; then
                            ...
                            log_message SUCCESS "元数据已回滚，建议执行 '重建所有 HTTP 配置' 同步状态。"
                        fi
                    fi ;;
            esac ;;
...
_manage_cron_jobs() {
    echo -e "\n${CYAN}--- 当前系统 Cron 任务 (acme.sh + 本助手) ---${NC}"
    crontab -l | grep -E "acme.sh|nginx_ssl_manager" || echo "无"
    echo ""
    if _confirm_action_or_exit_non_interactive "是否重新挂载定时续期任务 (推荐)?"; then
        "$ACME_BIN" --install-cronjob >/dev/null 2>&1 || true
        crontab -l | grep -v "$SCRIPT_PATH" > /tmp/cron.bak || true
        echo "0 3 * * * $SCRIPT_PATH --cron >> $LOG_FILE 2>&1" >> /tmp/cron.bak
        crontab /tmp/cron.bak; rm -f /tmp/cron.bak
        log_message SUCCESS "任务挂载成功。"
    fi
}
...
_rebuild_all_nginx_configs() {
    echo ""; log_message INFO "准备重建所有 HTTP Nginx 配置文件..."
    ...
    if _confirm_action_or_exit_non_interactive "是否开启 TLS/SSL 加密卸载 (TCP over TLS)?"; then
        ...
        while true; do
            ...
            if [ "$c_idx" -ge 1 ] && [ "$c_idx" -le "$idx" ]; then
                ssl_cert="${cert_list[$c_idx]}"; ssl_key="${key_list[$c_idx]}"; break
...
_gather_project_details() {
    ...
    if [ "$is_cert_only" == "false" ]; then
        if [ "$provider" == "dns_cf" ] || _confirm_action_or_exit_non_interactive "是否开启 Cloudflare 严格安全防御 (仅允许 CF 节点访问)?"; then cf_strict="y"; else cf_strict="n"; fi
    fi
...
_display_projects_list() {
    ...
    echo "$json" | jq -c '.[]' | while read -r p; do
        ...
        if [[ -f "$cert" ]]; then
            local end=$(openssl x509 -enddate -noout -in "$cert" 2>/dev/null | cut -d= -f2); local ts=$(date -d "$end" +%s 2>/dev/null || echo 0)
            ...
            else status_str="运行中"; status_color="$GREEN"; fi
...
_draw_dashboard() {
    ...
    echo -e "\n${GREEN}╭────────────────────────────────────────────────────────────────────────╮${NC}"
    echo -e "${GREEN}│${NC}                   ${BOLD}Nginx 管理面板 v4.24.0${NC}                   ${GREEN}│${NC}"
    echo -e "${GREEN}╰────────────────────────────────────────────────────────────────────────╯${NC}"
...
manage_configs() {
    while true; do
        ...
        _render_menu "管理: $selected_domain" "1. 查看证书详情" "2. 手动续期" "3. 删除项目" "4. 查看配置" "5. 查看日志" "6. 重新配置 (目标/限制等)" "7. 修改证书申请与续期设置 (不立即续期)" "8. 添加自定义指令"
...
_handle_reconfigure_project() {
    ...
    local skip_cert="true"; if _confirm_action_or_exit_non_interactive "是否重新申请/续期证书?"; then skip_cert="false"; fi
...
_handle_modify_renew_settings() {
    ...
    if _save_project_json "$new_json"; then
        log_message SUCCESS "证书续期设置已更新，将在证书快到期时自动应用。"
    else
...
_handle_cert_details() { local cert="$SSL_CERTS_BASE_DIR/${1:-}.cer"; if [ -f "$cert" ]; then echo -e "${CYAN}--- 证书详情 ---${NC}"; openssl x509 -in "$cert" -noout -text | grep -E "Issuer:|Not After|Subject:|DNS:"; echo -e "${CYAN}----------------${NC}"; else log_message ERROR "证书文件不存在。"; fi; press_enter_to_continue; }
...
main_menu() {
    while true; do
        ...
        case "$c" in
            ...
            7) _render_menu "查看日志" "1. Nginx 全局" "2. acme.sh 运行"
