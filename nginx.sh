【变更摘要】
`Full Release`: v4.33.1 正式版。
1.  **UI 全面优化**：整合了先前所有的布局修复。HTTP 项目管理列表现已实现动态居中对齐、列序调整（目标跟随域名）及醒目的颜色编码（亮黄/亮红预警）。
2.  **细节打磨**：移除了所有菜单框内的冗余空行，确保视觉紧凑。
3.  **功能完整**：包含 ACME 证书申请、TCP 负载均衡、CF 防御、备份还原及 Telegram 通知等全套功能。
