...
log_message() {
    ...
    local color_code="${RESET}" # Default to no color
    ...
    if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
        ...
        elif [ "$level" = "INFO" ]; then # INFO级别不显示前缀
            echo -e "${color_code}${message}${RESET}"
        else # 其他级别（WARN, ERROR）显示级别前缀
            echo -e "${color_code}[${level}] ${message}${RESET}"
        fi
    else # 非交互模式，所有级别都带前缀，纯文本
        echo "[${timestamp}] [${level}] ${message}" >> "$LOG_FILE"
    fi

    # 写入日志文件（纯文本，保留时间戳和所有级别）
    # 避免在非交互模式下重复写入（上面的else块已经写入了）
    if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
        echo "[${timestamp}] [${level}] ${message}" >> "$LOG_FILE"
    fi
}
...
find_acme_sh_path() {
    ...
    if command -v acme.sh &>/dev/null; then
        ...
        # 确保它是 acme.sh 脚本本身，而不是一个 wrapper
        if [[ "$path_from_cmd" == *".acme.sh/acme.sh"* || "$path_from_cmd" == *"/acme.sh" ]]; then
            echo "$path_from_cmd"
            return 0
        fi
    fi
...
get_vps_ip() {
    # VPS_IP 局部变量
    VPS_IP=$(curl -s -4 https://api.ipify.org || curl -s -4 https://ip.sb || echo "")
    if [[ -z "$VPS_IP" ]]; then
        log_message ERROR "❌ 无法获取 VPS 公网 IPv4 地址，请检查网络连接。"
        exit 1
    fi
    log_message INFO "🌐 VPS 公网 IP (IPv4): $VPS_IP"

    # VPS_IPV6 全局变量，不使用 local
    VPS_IPV6=$(curl -s -6 https://api64.ipify.org 2>/dev/null || curl -s -6 https://ip.sb 2>/dev/null || echo "")
    if [[ -n "$VPS_IPV6" ]]; then
...
generate_nginx_listen_directives() {
    ...
    local directives="    listen $port$([ -n "$ssl_http2_flags" ] && echo " $ssl_http2_flags");"
    if [[ -n "$VPS_IPV6" ]]; then # Use global VPS_IPV6 here
        directives+="\n    listen [::]:$port$([ -n "$ssl_http2_flags" ] && echo " $ssl_http2_flags");"
    fi
    echo -e "$directives"
}
...
_NGINX_HTTP_CHALLENGE_TEMPLATE() {
    local DOMAIN="$1"
    local LISTEN_80_DIRECTIVES="$2" # 从外部传入 listen 80;
...
${LISTEN_80_DIRECTIVES}
    ...
    location /.well-known/acme-challenge/ {
        root /var/www/html; # acme.sh webroot 验证目录的绝对路径
    }
...
_NGINX_FINAL_TEMPLATE() {
    ...
    local CUSTOM_SNIPPET_INCLUDE="$5" # 预处理好的 include 语句或空字符串
    local LISTEN_80_DIRECTIVES="$6"    # 从外部传入 listen 80;
    local LISTEN_443_DIRECTIVES="$7"   # 从外部传入 listen 443 ssl http2;
...
analyze_acme_error() {
    ...
    if echo "$error_output" | grep -q "Invalid response from"; then
        ...
        log_message ERROR "   可能原因：acme.sh 命令中使用了它不识别的参数。这可能是由于参数顺序错误、参数值引号问题或 acme.sh 版本过旧导致。"
        log_message YELLOW "   建议：确保您的 acme.sh 是最新版本 ('$ACME_BIN --upgrade')，并检查脚本中构建 acme.sh 命令的参数是否正确。"
    else
...
# -----------------------------
# 检查并清理 projects.json 文件
# 确保它是有效的 JSON 数组，并且没有无效或空的条目
clean_projects_metadata_file() {
    log_message INFO "🔍 检查并清理项目元数据文件 $PROJECTS_METADATA_FILE..."
    if [ ! -f "$PROJECTS_METADATA_FILE" ]; then
        echo "[]" > "$PROJECTS_METADATA_FILE"
        log_message GREEN "✅ 项目元数据文件 $PROJECTS_METADATA_FILE 已创建。"
        return 0
    fi

    local current_content=$(cat "$PROJECTS_METADATA_FILE")
    # Validate if it's a valid JSON array
    if ! jq -e . "$PROJECTS_METADATA_FILE" > /dev/null 2>&1; then
        log_message ERROR "❌ 警告: $PROJECTS_METADATA_FILE 不是有效的 JSON 格式。将尝试备份并重新创建。"
        mv "$PROJECTS_METADATA_FILE" "${PROJECTS_METADATA_FILE}.bak.$(date +%Y%m%d%H%M%S)"
        echo "[]" > "$PROJECTS_METADATA_FILE"
        log_message GREEN "✅ 项目元数据文件 $PROJECTS_METADATA_FILE 已重新创建。"
        return 0
    fi

    # Filter out null or invalid entries
    local cleaned_content=$(echo "$current_content" | jq '.[] | select(type == "object" and .domain != null and .domain != "")')
    # If cleaned_content is empty (no valid objects), jq will output nothing. We need to explicitly make it "[]".
    if [[ -z "$cleaned_content" ]]; then
        echo "[]" > "$PROJECTS_METADATA_FILE"
        log_message GREEN "✅ 项目元数据文件 $PROJECTS_METADATA_FILE 已清理，移除了所有无效条目。"
    else
        echo "[$cleaned_content]" | jq '.' > "${PROJECTS_METADATA_FILE}.tmp"
        mv "${PROJECTS_METADATA_FILE}.tmp" "$PROJECTS_METADATA_FILE"
        log_message GREEN "✅ 项目元数据文件 $PROJECTS_METADATA_FILE 已清理并更新。"
    fi
    sleep 1
    return 0
}
...
configure_nginx_projects() {
    ...
    clean_projects_metadata_file # 确保元数据文件是健康的
    ...
    if [ "$ACME_CA_SERVER_NAME" = "zerossl" ]; then
        ...
        if ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then
             ...
             local acme_register_cmd=( "$ACME_BIN" --register-account -m "$ZERO_SSL_ACCOUNT_EMAIL" --server "$ACME_CA_SERVER_URL" )
             if ! "${acme_register_cmd[@]}"; then
    ...
    for P in "${PROJECTS[@]}"; do
        ...
        if [ "$DOCKER_INSTALLED" = true ] && docker ps --format '{{.Names}}' | grep -wq "$TARGET_INPUT"; then
            ...
            if [[ -n "$HOST_MAPPED_PORT" ]]; then
                ...
                # Use jq to robustly parse exposed ports
                while IFS= read -r port_entry; do
                    INTERNAL_EXPOSED_PORTS_ARRAY+=("$port_entry")
                done < <(docker inspect "$TARGET_INPUT" --format '{{.Config.ExposedPorts | keys | .[]}}' 2>/dev/null | sed 's|/tcp||g')
        ...
        if [[ -f "$INSTALLED_CRT_FILE" && -f "$INSTALLED_KEY_FILE" ]]; then
            ...
            local EXISTING_END_TS=0
            # Try parsing with GNU date first
            if command -v date >/dev/null 2>&1 && date -d "$EXISTING_END_DATE" +"%s" &>/dev/null; then
                EXISTING_END_TS=$(date -d "$EXISTING_END_DATE" +%s 2>/dev/null)
            # Fallback to BSD date if GNU date fails
            elif command -v date >/dev/null 2>&1 && date -j -f "%b %d %T %Y %Z" "$EXISTING_END_DATE" "+%s" &>/dev/null; then
                 EXISTING_END_TS=$(date -j -f "%b %d %T %Y %Z" "$EXISTING_END_DATE" "+%s" 2>/dev/null)
            fi
            EXISTING_END_TS=${EXISTING_END_TS:-0} # Default to 0 if parsing fails

            local NOW_TS=$(date +%s)
            local EXISTING_LEFT_DAYS=0
            if (( EXISTING_END_TS > NOW_TS )); then
                EXISTING_LEFT_DAYS=$(( (EXISTING_END_TS - NOW_TS) / 86400 ))
            fi
        ...
        local NEW_PROJECT_JSON=$(jq -n \
            ...
            --arg custom_snippet "${CUSTOM_NGINX_SNIPPET_FILE:-null}" \
            --arg acme_method "$ACME_VALIDATION_METHOD" \
            --arg dns_provider "${DNS_API_PROVIDER:-null}" \
        ...
        local CUSTOM_SNIPPET_INCLUDE=""
        if [[ -n "$CUSTOM_NGINX_SNIPPET_FILE" && -f "$CUSTOM_NGINX_SNIPPET_FILE" ]]; then
            CUSTOM_SNIPPET_INCLUDE="    include $CUSTOM_NGINX_SNIPPET_FILE;"
        fi
        ...
        if [ "$SHOULD_ISSUE_CERT" = "y" ]; then
            ...
            if ! "${ACME_INSTALL_CMD_ARRAY[@]}"; then log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; continue; fi
        else
        ...
        _NGINX_FINAL_TEMPLATE "$MAIN_DOMAIN" "$PROXY_TARGET_URL" "$INSTALLED_CRT_FILE" "$INSTALLED_KEY_FILE" "$CUSTOM_SNIPPET_INCLUDE" "$LISTEN_80_DIRECTIVES" "$LISTEN_443_DIRECTIVES" > "$DOMAIN_CONF"
...
import_existing_project() {
    ...
    if [[ -n "$USER_TARGET_INPUT" ]]; then
        if [ "$DOCKER_INSTALLED" = true ] && docker ps --format '{{.Names}}' | grep -wq "$USER_TARGET_INPUT"; then
            ...
            if [[ -n "$HOST_MAPPED_PORT" ]]; then
                ...
                while IFS= read -r port_entry; do
                    INTERNAL_EXPOSED_PORTS_ARRAY+=("$port_entry")
                done < <(docker inspect "$USER_TARGET_INPUT" --format '{{.Config.ExposedPorts | keys | .[]}}' 2>/dev/null | sed 's|/tcp||g')
    ...
    local NEW_PROJECT_JSON=$(jq -n \
        ...
        --arg custom_snippet "${IMPORTED_CUSTOM_SNIPPET:-null}" \
...
manage_configs() {
    ...
    clean_projects_metadata_file # 确保文件是健康的，尤其是在进入管理界面时
    ...
    echo "$PROJECTS_ARRAY_RAW" | while read -r project_json; do
        INDEX=$((INDEX + 1))
        ...
        local PROJECT_TYPE=$(echo "$project_json" | jq -r '.type // "unknown"')
        local PROJECT_NAME=$(echo "$project_json" | jq -r '.name // "未知"')
        local RESOLVED_PORT=$(echo "$project_json" | jq -r '.resolved_port // "N/A"')
        ...
        if [[ -f "$CERT_FILE" && -f "$KEY_FILE" ]]; then
            ...
            local END_TS=0
            # Try parsing with GNU date first
            if command -v date >/dev/null 2>&1 && date -d "$END_DATE" +"%s" &>/dev/null; then
                END_TS=$(date -d "$END_DATE" +%s 2>/dev/null)
            # Fallback to BSD date if GNU date fails
            elif command -v date >/dev/null 2>&1 && date -j -f "%b %d %T %Y %Z" "$END_DATE" "+%s" &>/dev/null; then
                 END_TS=$(date -j -f "%b %d %T %Y %Z" "$END_DATE" "+%s" 2>/dev/null)
            fi
            END_TS=${END_TS:-0} # Default to 0 if parsing fails


            if (( END_TS > 0 )); then # Only calculate if END_TS is valid
                local NOW_TS=$(date +%s)
                LEFT_DAYS=$(( (END_TS - NOW_TS) / 86400 ))

                # Try to format date with GNU or BSD date
                if command -v date >/dev/null 2>&1 && date -d "$END_DATE" +"%Y年%m月%d日" &>/dev/null; then
                    FORMATTED_END_DATE=$(date -d "$END_DATE" +"%Y年%m月%d日" 2>/dev/null)
                elif command -v date >/dev/null 2>&1 && date -j -f "%b %d %T %Y %Z" "$END_DATE" "+%Y年%m月%d日" &>/dev/null; then
        ...
        case "$MANAGE_CHOICE" in
            ...
            3) # 编辑项目核心配置 (不含片段)
                ...
                if [[ -n "$NEW_TARGET_INPUT" ]]; then
                    ...
                    if [ "$DOCKER_INSTALLED" = true ] && docker ps --format '{{.Names}}' | grep -wq "$NEW_TARGET_INPUT"; then
                        ...
                        if [[ -n "$HOST_MAPPED_PORT" ]]; then
                            ...
                            while IFS= read -r port_entry; do
                                INTERNAL_EXPOSED_PORTS_ARRAY+=("$port_entry")
                            done < <(docker inspect "$NEW_TARGET_INPUT" --format '{{.Config.ExposedPorts | keys | .[]}}' 2>/dev/null | sed 's|/tcp||g')
                            ...
                            if [ ${#INTERNAL_EXPOSED_PORTS_ARRAY[@]} -gt 0 ]; then
                                ...
                                while true; do
                                    ...
                                    if [[ "$USER_INTERNAL_PORT_EDIT" =~ ^[0-9]+$ && "$USER_INTERNAL_PORT_EDIT" -gt 0 && "$USER_INTERNAL_PORT_EDIT" -lt 65536 ]]; then NEW_RESOLVED_PORT="$USER_INTERNAL_PORT_EDIT"; FINAL_PROXY_TARGET_URL="http://127.0.0.1:$NEW_RESOLVED_PORT"; log_message GREEN "✅ 已指定容器内部端口: $NEW_RESOLVED_PORT。"; break; else log_message RED "端口无效"; fi; done
                            fi
                        fi
                ...
                if [[ "$EDIT_ACME_VALIDATION_METHOD" = "imported" || "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]]; then
                    ...
                    if [ "$NEW_CA_SERVER_NAME" = "zerossl" ]; then
                         ...
                         if ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then
                            ...
                            local acme_register_cmd_edit=( "$ACME_BIN" --register-account -m "$NEW_ZERO_SSL_ACCOUNT_EMAIL" --server "$NEW_CA_SERVER_URL" )
                            if ! "${acme_register_cmd_edit[@]}"; then
                                log_message ERROR "❌ ZeroSSL 账户注册失败！请检查邮箱地址或稍后重试。"
                                sleep 2
                                continue # 返回编辑菜单
                            fi
                ...
                local UPDATED_PROJECT_JSON=$(jq -n \
                    ...
                    --arg custom_snippet "${EDIT_CUSTOM_SNIPPET_ORIGINAL:-null}" \
                    --arg acme_method "$NEW_ACME_VALIDATION_METHOD" \
                    --arg dns_provider "${NEW_DNS_API_PROVIDER:-null}" \
                ...
                if [ "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]; then
                    ...
                    if [[ "$UPDATE_NOW" =~ ^[Yy]$ ]]; then
                        ...
                        local CUSTOM_SNIPPET_INCLUDE_EDIT=""
                        if [[ -n "$EDIT_CUSTOM_SNIPPET_ORIGINAL" && -f "$EDIT_CUSTOM_SNIPPET_ORIGINAL" ]]; then
                            CUSTOM_SNIPPET_INCLUDE_EDIT="    include $EDIT_CUSTOM_SNIPPET_ORIGINAL;"
                        fi
                        ...
                        local INSTALL_CERT_DOMAINS_ARRAY_EDIT=()
                        INSTALL_CERT_DOMAINS_ARRAY_EDIT+=( -d "$DOMAIN_TO_EDIT" )
                        if [ "$NEW_USE_WILDCARD" = "y" ]; then
                            INSTALL_CERT_DOMAINS_ARRAY_EDIT+=( -d "*.$DOMAIN_TO_EDIT" ) # Wildcard should be literal `*.domain`
                        fi
                        # acme.sh 会自动执行 --reloadcmd
                        local ACME_INSTALL_CMD_ARRAY_EDIT=(
                            "$ACME_BIN" --install-cert "${INSTALL_CERT_DOMAINS_ARRAY_EDIT[@]}" --ecc
                            --key-file "$NEW_KEY_FILE"
                            --fullchain-file "$NEW_CERT_FILE"
                            --reloadcmd "systemctl reload nginx"
                        )
                        if ! "${ACME_INSTALL_CMD_ARRAY_EDIT[@]}"; then log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; sleep 2; continue; fi

                        log_message YELLOW "生成 $DOMAIN_TO_EDIT 的最终 Nginx 配置..."
                        _NGINX_FINAL_TEMPLATE "$DOMAIN_TO_EDIT" "$FINAL_PROXY_TARGET_URL" "$NEW_CERT_FILE" "$NEW_KEY_FILE" "$CUSTOM_SNIPPET_INCLUDE_EDIT" "$LISTEN_80_DIRECTIVES_EDIT" "$LISTEN_443_DIRECTIVES_EDIT" > "$NGINX_SITES_AVAILABLE_DIR/$DOMAIN_TO_EDIT.conf"
            ...
            4) # 管理自定义 Nginx 配置片段
                ...
                if [[ "$CHOSEN_SNIPPET_PATH" != "$CURRENT_SNIPPET_PATH" || "$RELOAD_NGINX_AFTER_UPDATE" = "y" ]]; then
                    local UPDATED_SNIPPET_JSON_OBJ=$(jq -n --arg custom_snippet "${CHOSEN_SNIPPET_PATH:-null}" '{custom_snippet: $custom_snippet}')
                    ...
                    local CUSTOM_SNIPPET_INCLUDE_SNIPPET=""
                    if [[ -n "$CHOSEN_SNIPPET_PATH" && -f "$CHOSEN_SNIPPET_PATH" ]]; then
                        CUSTOM_SNIPPET_INCLUDE_SNIPPET="    include $CHOSEN_SNIPPET_PATH;"
                    fi

                    log_message YELLOW "正在重新生成 $DOMAIN_FOR_SNIPPET 的 Nginx 配置..."
                    _NGINX_FINAL_TEMPLATE "$DOMAIN_FOR_SNIPPET" "$PROXY_TARGET_URL_SNIPPET" "$CERT_FILE_SNIPPET" "$KEY_FILE_SNIPPET" "$CUSTOM_SNIPPET_INCLUDE_SNIPPET" "$LISTEN_80_DIRECTIVES_SNIPPET" "$LISTEN_443_DIRECTIVES_SNIPPET" > "$DOMAIN_CONF_SNIPPET"
...
check_and_auto_renew_certs() {
    ...
    clean_projects_metadata_file # 确保文件是健康的
    ...
    jq -c '.[] | select(type == "object" and .domain != null and .domain != "")' "$PROJECTS_METADATA_FILE" | while read -r project_json; do
        local DOMAIN=$(echo "$project_json" | jq -r '.domain')
        local ACME_VALIDATION_METHOD=$(echo "$project_json" | jq -r '.acme_validation_method')
        local DNS_API_PROVIDER=$(echo "$project_json" | jq -r '.dns_api_provider // "null"')
        ...
        # Try parsing date with GNU date first
        if command -v date >/dev/null 2>&1 && date -d "$END_DATE" +"%s" &>/dev/null; then
            END_TS=$(date -d "$END_DATE" +%s 2>/dev/null)
        # Fallback to BSD date if GNU date fails
        elif command -v date >/dev/null 2>&1 && date -j -f "%b %d %T %Y %Z" "$END_DATE" "+%s" &>/dev/null; then
        ...
        END_TS=${END_TS:-0} # Default to 0 if parsing fails or invalid
    ...
    while true; do
        ...
        case "$ACCOUNT_CHOICE" in
            ...
            2)
                ...
                local acme_register_cmd_new=( "$ACME_BIN" --register-account -m "$NEW_ACCOUNT_EMAIL" --server "$REGISTER_CA_SERVER_URL" )
                if "${acme_register_cmd_new[@]}"; then
            ...
            3)
                ...
                local acme_set_default_cmd=( "$ACME_BIN" --set-default-account -m "$DEFAULT_ACCOUNT_EMAIL" )
                if "${acme_set_default_cmd[@]}"; then
