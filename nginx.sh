#!/usr/bin/env bash
# =============================================================
# 🚀 Nginx 反向代理 + HTTPS 证书管理助手 (v4.33.1 - UI Final)
# =============================================================
# 作者：Shell 脚本专家
...
readonly BRIGHT_RED="\033[91m"      # 亮红：用于报警/过期
readonly BRIGHT_YELLOW="\033[93m"   # 亮黄：用于预警/续期
readonly GRAY="\033[2m"             # 灰色：用于非关键信息
...
check_dependencies() {
    local -a required=(nginx curl socat openssl jq idn wc sed awk grep date uptime find mktemp tar systemctl realpath flock)
    local -a missing=()
    local cmd
...
_validate_reload_cmd() {
    ...
    if ! _is_safe_shell_command "$cmd"; then
        return 1
    fi
    if [[ "$cmd" =~ ^systemctl[[:space:]]+restart[[:space:]]+[a-zA-Z0-9@_.-]+$ ]]; then
        return 0
    fi
    if [ "$cmd" = "systemctl reload nginx" ]; then
        return 0
    fi
    return 1
}
...
_center_text() {
    local text="$1"
    local width="${2:-10}"
    # 获取原始长度。使用原生 Bash 变量长度计算最快（微秒级）。
    # 注意：在 Bash 中，一个中文字符算作长度 1。
    local len=${#text}
...
_draw_dashboard() {
    ...
    local title="Nginx 管理面板 v4.33.1"
...
_validate_target_list() {
    local target="${1:-}"
    if [[ ! "$target" =~ ^[a-zA-Z0-9.-]+:[0-9]+(,[a-zA-Z0-9.-]+:[0-9]+)*$ ]]; then
        return 1
    fi
    local item
    IFS=',' read -ra ADDR <<< "$target"
    for item in "${ADDR[@]}"; do
        if ! _validate_ip_or_hostname_port "$item"; then
            return 1
        fi
    done
    return 0
...
_validate_custom_config_block() {
    ...
    while IFS= read -r line; do
        if ! _validate_nginx_directive "$line"; then
            return 1
        fi
        if ! _is_allowed_custom_directive "$line"; then
            return 1
        fi
    done <<< "$block"
    return 0
...
setup_tg_notifier() {
    ...
    local tk
    if ! tk=$(_prompt_user_input_with_validation "请输入 Bot Token (如 1234:ABC...)" "$real_tk_default" "^[0-9]+:[A-Za-z0-9_-]+$" "格式错误" "false" "$vis_tk_default"); then return; fi
    ...
    if ! _is_safe_path "$TG_CONF_FILE"; then
        log_message ERROR "配置路径不安全，已取消写入。"
        return
    fi
...
install_acme_sh() {
    ...
    local -a cmd=(/bin/sh -c "curl -fsSL https://get.acme.sh | /bin/sh")
    if [ -n "$email" ]; then
        if _validate_email "$email"; then
            cmd=(curl -fsSL https://get.acme.sh)
            if ! "${cmd[@]}" | /bin/sh -s -- --email "$email"; then
...
control_nginx() {
    ...
    _report_duration "Nginx ${action}" "$start_ts"
    return 0
}
...
_delete_project_json() {
    ...
    jq --arg d "${1:-}" 'del(.[] | select(.domain == $d))' "$PROJECTS_METADATA_FILE" > "$temp"
    if [ $? -eq 0 ]; then
        mv "$temp" "$PROJECTS_METADATA_FILE"
        return 0
    fi
...
_write_and_enable_nginx_config() {
    ...
    local tmp_conf tmp_enabled
    tmp_conf=$(mktemp "${NGINX_SITES_AVAILABLE_DIR}/.${domain}.conf.XXXXXX")
    tmp_enabled="${NGINX_SITES_ENABLED_DIR}/.${domain}.conf.tmp"
    cat > "$tmp_conf" << EOF
server {
    listen 80;
...
EOF
    ln -sf "$tmp_conf" "$tmp_enabled"
    if nginx -t >/dev/null 2>&1; then
        mv "$tmp_conf" "$conf"
        ln -sf "$conf" "$NGINX_SITES_ENABLED_DIR/"
        rm -f "$tmp_enabled"
    else
        rm -f "$tmp_conf" "$tmp_enabled"
...
_write_and_enable_tcp_config() {
    ...
    if ! _validate_target_list "$target"; then
    ...
    local tmp_conf tmp_enabled
    tmp_conf=$(mktemp "${NGINX_STREAM_AVAILABLE_DIR}/.tcp_${port}.conf.XXXXXX")
    tmp_enabled="${NGINX_STREAM_ENABLED_DIR}/.tcp_${port}.conf.tmp"
    cat > "$tmp_conf" << EOF
${upstream_block}
server {
...
configure_tcp_proxy() {
    ...
    if ! _validate_port "$l_port"; then log_message ERROR "无效端口: ${l_port}"; return; fi
    ...
    if ! _validate_target_list "$target"; then log_message ERROR "目标地址格式无效"; return; fi
        ...
        case "$cc" in
            1) 
                if _confirm_action_or_exit_non_interactive "确认删除 TCP 代理 $selected_port？"; then
                    ...
                    _snapshot_projects_json "$TCP_PROJECTS_METADATA_FILE"
                    local temp=$(mktemp)
                    jq --arg p "$selected_port" 'del(.[] | select(.listen_port == $p))' "$TCP_PROJECTS_METADATA_FILE" > "$temp"
                    if [ $? -eq 0 ]; then
                        mv "$temp" "$TCP_PROJECTS_METADATA_FILE"
                    else
                        rm -f "$temp"
...
EOF
                ...
                mkdir -p "$NGINX_WEBROOT_DIR"; cmd+=("--webroot" "$NGINX_WEBROOT_DIR")
                 ...
                 if _confirm_action_or_exit_non_interactive "是否临时停止 $temp_svc 以释放 80 端口?"; then
                    systemctl stop "$temp_svc"; STOPPED_SERVICE="$temp_svc"
                 fi
                 cmd+=("--standalone")
            fi
        else
...
_gather_project_details() {
    ...
    if ! _validate_domain "$domain"; then
        log_message ERROR "域名格式无效: ${domain}" >&2
        exec 1>&3
        return 1
    fi
        ...
        if [ "$skip_cert" == "false" ]; then
            ...
            case "$hk" in
                ...
                5) if ! reload_cmd=$(_prompt_user_input_with_validation "请输入完整 Shell 命令" "" "" "" "true"); then exec 1>&3; return 1; fi
                   if ! _validate_reload_cmd "$reload_cmd"; then
                       log_message ERROR "自定义命令不安全，仅允许 systemctl restart <service> 或 systemctl reload nginx。"
                       exec 1>&3
                       return 1
                   fi ;;
                6) reload_cmd="" ;;
    ...
    echo "$json" | jq -c '.[]' | while read -r p; do
        ...
        if [[ ! -f "$cert" ]]; then
            ...
            if (( days < 0 )); then 
                ...
                color_code="${BRIGHT_RED}" # 亮红：醒目报警
            elif (( days <= 30 )); then 
                status_text="${days}天续期"
                color_code="${BRIGHT_YELLOW}" # 亮黄：醒目警告
            else 
        ...
        # 4. 状态 (居中 + 颜色)
        # 手动处理居中以确保颜色代码不影响 padding 计算
        local status_len=${#status_text} # 纯文本长度
...
_handle_set_custom_config() {
    ...
    if [ "$new_val" == "clear" ]; then
        ...
        if ! _validate_custom_config_block "$new_val"; then
            log_message ERROR "自定义指令不安全或格式错误（每行需以分号结尾，且禁止控制字符/重定向/管道/花括号）。"
            press_enter_to_continue
            return
        fi
    fi
...
main_menu() {
    while true; do
        ...
        echo -e " 8. ${BRIGHT_YELLOW}${BOLD}更新 Cloudflare 防御 IP 库 (强烈建议防源站被扫配置)${NC}"
