...
_render_nginx_template_root_menu() {
  _render_menu "配置模板中心" \
    "1. 通用反代（security_headers + reverse_proxy_enhanced）" \
    "2. HTTPS生产（security_headers + hsts）" \
    "3. WordPress常用（wordpress_basic + security_headers）" \
    "4. 长连接服务（reverse_proxy_enhanced + security_headers）" \
    "5. 自定义模板（单项，可多选）" \
    "6. 清理模板配置" \
    "7. 返回"
}
...
_render_nginx_template_custom_menu() {
  _render_menu "自定义模板（单项，可多选）" \
    "1. security_headers" \
    "2. hsts" \
    "3. reverse_proxy_enhanced" \
    "4. wordpress_basic" \
    "5. 返回"
}
...
_template_combo_by_choice() {
  local choice="${1:-}"
  case "$choice" in
  1) printf '%s\n' "security_headers reverse_proxy_enhanced" ;;
  2) printf '%s\n' "security_headers hsts" ;;
  3) printf '%s\n' "wordpress_basic security_headers" ;;
  4) printf '%s\n' "reverse_proxy_enhanced security_headers" ;;
  *) return 1 ;;
  esac
}
...
_parse_custom_template_multi_select() {
  ...
  for token in "${parts[@]}"; do
    case "$token" in
    1) id="security_headers" ;;
    2) id="hsts" ;;
    3) id="reverse_proxy_enhanced" ;;
    4) id="wordpress_basic" ;;
    5) continue ;;
    *)
