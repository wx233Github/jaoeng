    ...
    while true; do  
        ...
        case "$MANAGE_CHOICE" in  
            ...
            2) # 删除  
                ...
                if [[ -z "$DOMAIN_TO_DELETE" ]]; then log_message RED "❌ 域名不能为空！"; sleep 1; continue; conflg; fi  
                ...
                if [ "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]; then  
                    ...
                    if [[ "$UPDATE_NOW" =~ ^[Yy]$ ]]; then  
                        ...
                        eval "$ACME_BIN --install-cert $INSTALL_CERT_DOMAINS --ecc \  
                            ...
                            --reloadcmd \"systemctl reload nginx\"" || { log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; sleep 2; continue; }  
