...
log_message() {    
    ...
    local color_code="${RESET}" # Default color is reset    
    ...
    if [ "$IS_INTERACTIVE_MODE" = "true" ]; then  
        if [ "$level" = "DEBUG" ]; then
            ...
            # For non-DEBUG, just output the colored message without prefix.
            # Using printf with %b for color interpretation in case echo -e has issues.
            printf "${color_code}%b${RESET}\n" "$message"
        fi
    fi  
...
# This function's output will be captured into a variable BEFORE writing to the Nginx config.    
generate_nginx_listen_directives_output() {    
    local port="$1"    
...
_NGINX_HTTP_CHALLENGE_TEMPLATE() {    
    local DOMAIN="$1"    
    local LISTEN_80_DIRECTIVES=$(generate_nginx_listen_directives_output 80 "") # Capture output here    
...
_NGINX_FINAL_TEMPLATE() {    
    ...
    local LISTEN_80_DIRECTIVES=$(generate_nginx_listen_directives_output 80 "") # Capture output here    
    local LISTEN_443_DIRECTIVES=$(generate_nginx_listen_directives_output 443 "ssl http2") # Capture output here    
    ...
    for P in "${PROJECTS[@]}"; do    
        ...
        if [ "$DOCKER_INSTALLED" = true ] && docker ps --format '{{.Names}}' | grep -wq "$TARGET_INPUT"; then    
            ...
            local HOST_MAPPED_PORT=$(docker inspect "$TARGET_INPUT" --format \
                '{{- range $p, $conf := .NetworkSettings.Ports -}}{{- if $conf -}}{{- (index $conf 0).HostPort -}}{{- end -}}{{- end -}}' 2>/dev/null | grep -Eo '[0-9]+' | head -n1 || echo "")
            ...
            if [[ -n "$HOST_MAPPED_PORT" ]]; then    
                ...
                # Use jq for more robust parsing of exposed ports
                while IFS= read -r port_entry; do
                    INTERNAL_EXPOSED_PORTS_ARRAY+=("$port_entry")
                done < <(docker inspect "$TARGET_INPUT" --format '{{json .Config.ExposedPorts}}' 2>/dev/null | jq -r 'keys[] | split("/")[0]' || echo "")
        ...
        if [ "$SHOULD_ISSUE_CERT" = "y" ] && [ "$ACME_VALIDATION_METHOD" = "http-01" ]; then    
            ...
            local DOMAIN_CONF_FILE="$NGINX_SITES_AVAILABLE_DIR/$MAIN_DOMAIN.conf"    
            _NGINX_HTTP_CHALLENGE_TEMPLATE "$MAIN_DOMAIN" > "$DOMAIN_CONF_FILE"    
                
            if [ ! -L "$NGINX_SITES_ENABLED_DIR/$MAIN_DOMAIN.conf" ]; then    
                ln -sf "$DOMAIN_CONF_FILE" "$NGINX_SITES_ENABLED_DIR/"    
            fi    
    
            if ! control_nginx restart; then    
                log_message ERROR "❌ Nginx 重启失败，证书申请将无法进行。清理临时配置并跳过域名 $MAIN_DOMAIN。"    
                rm -f "$DOMAIN_CONF_FILE"    
                rm -f "$NGINX_SITES_ENABLED_DIR/$MAIN_DOMAIN.conf"    
                if jq -e ".[] | select(.domain == \"$MAIN_DOMAIN\")" "$PROJECTS_METADATA_FILE" > /dev/null; then    
        ...
        if [ "$SHOULD_ISSUE_CERT" = "y" ]; then    
            ...
            ACME_ISSUE_COMMAND_ARRAY+=("--debug" "2") # Add debug for more verbose logging    
            ...
            if ! "${ACME_INSTALL_COMMAND_ARRAY[@]}"; then    
                ...
                # Handle cleanup if install fails    
                rm -f "$DOMAIN_CONF"    
                rm -f "$NGINX_SITES_ENABLED_DIR/$MAIN_DOMAIN.conf"    
    ...
    if [[ -n "$USER_TARGET_INPUT" ]]; then    
        if [ "$DOCKER_INSTALLED" = true ] && docker ps --format '{{.Names}}' | grep -wq "$USER_TARGET_INPUT"; then    
            ...
            if [[ -n "$HOST_MAPPED_PORT" ]]; then    
                ...
                while IFS= read -r port_entry; do    
                    INTERNAL_EXPOSED_PORTS_ARRAY+=("$port_entry")    
                done < <(docker inspect "$USER_TARGET_INPUT" --format '{{json .Config.ExposedPorts}}' 2>/dev/null | jq -r 'keys[] | split("/")[0]' || echo "")    
                ...
                if [[ -n "$NEW_TARGET_INPUT" ]]; then    
                    ...
                    if [ "$DOCKER_INSTALLED" = true ] && docker ps --format '{{.Names}}' | grep -wq "$NEW_TARGET_INPUT"; then    
                        ...
                        if [[ -n "$HOST_MAPPED_PORT" ]]; then    
                            ...
                            while IFS= read -r port_entry; do    
                                INTERNAL_EXPOSED_PORTS_ARRAY+=("$port_entry")    
                            done < <(docker inspect "$NEW_TARGET_INPUT" --format '{{json .Config.ExposedPorts}}' 2>/dev/null | jq -r 'keys[] | split("/")[0]' || echo "")    
                ...
                if [[ "$EDIT_ACME_VALIDATION_METHOD" = "imported" || "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]]; then    
                    ...
                    if [ "$NEW_CA_SERVER_NAME" = "zerossl" ]; then    
                         ...
                         if ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then    
                            ...
                            local register_new_cmd=("$ACME_BIN" "--register-account" "-m" "$NEW_ZERO_SSL_ACCOUNT_EMAIL" "--server" "$NEW_CA_SERVER_URL")    
                            "${register_new_cmd[@]}" || {    
                                log_message ERROR "❌ ZeroSSL 账户注册失败！请检查邮箱地址或稍后重试。"    
                                sleep 2    
                                continue # 返回编辑菜单  
                ...
                if [ "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]; then    
                    ...
                    if [[ "$UPDATE_NOW" =~ ^[Yy]$ ]]; then    
                        ...
                        local ACME_INSTALL_COMMAND_ARRAY=("$ACME_BIN" "--install-cert" "${INSTALL_CERT_DOMAINS_ARRAY[@]}" "--ecc" \    
                            ...
                            "--reloadcmd" "systemctl reload nginx")    
                        if ! "${ACME_INSTALL_COMMAND_ARRAY[@]}"; then log_message ERROR "❌ acme.sh 证书安装或Nginx重载失败。"; sleep 2; continue; fi    
...
main_menu() {    
    while true; do    
        ...
        printf "${CYAN}╚═══════════════════════════════════════╝${RESET}\n"    
        printf "\n" # 添加空行美化
        printf "${GREEN}  1) 配置新的 Nginx 反向代理和 HTTPS 证书${RESET}\n"    
        printf "${GREEN}  2) 查看与管理已配置项目 (域名、端口、证书)${RESET}\n"    
        printf "${GREEN}  3) 检查并自动续期所有证书${RESET}\n"    
        printf "${GREEN}  4) 管理 acme.sh 账户${RESET}\n"    
