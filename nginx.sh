...
log_message() {
    ...
    if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
        if [ "$level" = "DEBUG" ]; then
            printf "${color_code}[%s] %s%b\n" "$level" "$message" "$RESET"
        else
            printf "${color_code}%s%b\n" "$message" "$RESET" # 只显示消息，不带级别前缀
        fi
    else # 非交互模式下，所有级别都显示前缀，方便日志分析
        printf "[%s] %s %b\n" "$level" "$message" "$RESET"
    fi
...
install_acme_sh() {
    ...
    if [ ! -f "$ACME_BIN" ]; then
        ...
        local ACME_INSTALL_CMD=(curl https://get.acme.sh)
        if [[ -n "$ACME_EMAIL" ]]; then
            ACME_INSTALL_CMD+=("| sh -s email=\"$ACME_EMAIL\"") # Special handling for curl pipe
            log_message BLUE "➡️ 正在使用邮箱 $ACME_EMAIL 安装 acme.sh..."
        else
            ACME_INSTALL_CMD+=("| sh") # Special handling for curl pipe
        ...
        # Execute curl pipe with eval, this is one of the few cases where eval is necessary for pipes
        if eval "${ACME_INSTALL_CMD[*]}"; then
            log_message GREEN "✅ acme.sh 安装成功。"
        else
...
configure_nginx_projects() {
    ...
    if [ "$ACME_CA_SERVER_NAME" = "zerossl" ]; then
        ...
        if ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then
             ...
             local REGISTER_ZERO_SSL_CMD=("$ACME_BIN" --register-account -m "$ZERO_SSL_ACCOUNT_EMAIL" --server "$ACME_CA_SERVER_URL")
             if ! "${REGISTER_ZERO_SSL_CMD[@]}"; then
    ...
    for P in "${PROJECTS[@]}"; do
        ...
        if [ "$DOCKER_INSTALLED" = true ] && docker ps --format '{{.Names}}' | grep -wq "$TARGET_INPUT"; then
            ...
            if [[ -n "$HOST_MAPPED_PORT" ]]; then
                ...
                # Use jq to safely extract exposed ports
        ...
        if [ "$SHOULD_ISSUE_CERT" = "y" ] && [ "$ACME_VALIDATION_METHOD" = "http-01" ]; then
            ...
            local DOMAIN_CONF_HTTP="$NGINX_SITES_AVAILABLE_DIR/$MAIN_DOMAIN.conf" # Use a distinct variable name
            _NGINX_HTTP_CHALLENGE_TEMPLATE "$MAIN_DOMAIN" > "$DOMAIN_CONF_HTTP"

            if [ ! -L "$NGINX_SITES_ENABLED_DIR/$MAIN_DOMAIN.conf" ]; then
                ln -sf "$DOMAIN_CONF_HTTP" "$NGINX_SITES_ENABLED_DIR/"
            fi

            if ! control_nginx restart; then
                log_message ERROR "❌ Nginx 重启失败，证书申请将无法进行。清理临时配置并跳过域名 $MAIN_DOMAIN。"
                rm -f "$DOMAIN_CONF_HTTP"
                rm -f "$NGINX_SITES_ENABLED_DIR/$MAIN_DOMAIN.conf"
                if jq -e ".[] | select(.domain == \"$MAIN_DOMAIN\")" "$PROJECTS_METADATA_FILE" > /dev/null; then
        ...
        if [ "$SHOULD_ISSUE_CERT" = "y" ]; then
            ...
            # Build command array for acme.sh --issue
            ...
            # Build command array for acme.sh --install-cert
    ...
    while true; do
        ...
        case "$MANAGE_CHOICE" in
            1) # 手动续期
                ...
                if [ -z "$RENEW_PROJECT_JSON" ]; then log_message RED "❌ 域名 $DOMAIN_TO_RENEW 未找到在已配置列表中。"; sleep 1; continue;渐近符号
                fi
                ...
                # Build command array for acme.sh --renew
            ...
            2) # 删除
                ...
                if [[ "$CONFIRM_DELETE" =~ ^[Yy]$ ]]; then
                    ...
                    if [ "$delete_certs" = true ]; then
                        ...
                        # Build command array for acme.sh --remove
                        local REMOVE_CMD_ARRAY=("$ACME_BIN" --remove -d "$DOMAIN_TO_DELETE" --ecc)
                        "${REMOVE_CMD_ARRAY[@]}" 2>/dev/null || true
            ...
            3) # 编辑项目核心配置 (不含片段)
                ...
                local EDIT_KEY_FILE=$(echo "$CURRENT_PROJECT_JSON" | jq -r --arg default_key "$default_key_file_edit" '.key_file // $default_key') # Corrected variable name

                if [[ -z "$EDIT_CERT_FILE" || "$EDIT_CERT_FILE" == "null" ]]; then EDIT_CERT_FILE="$default_cert_file_edit"; fi
                if [[ -z "$EDIT_KEY_FILE" || "$EDIT_KEY_FILE" == "null" ]]; then EDIT_KEY_FILE="$default_key_file_edit"; fi # Corrected variable name
                ...
                if [[ "$EDIT_ACME_VALIDATION_METHOD" = "imported" || "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]]; then
                    ...
                    if [ "$NEW_CA_SERVER_NAME" = "zerossl" ]; then
                         ...
                         if ! "$ACME_BIN" --list | grep -q "ZeroSSL.com"; then
                            ...
                            local REGISTER_ZERO_SSL_CMD_EDIT=("$ACME_BIN" --register-account -m "$NEW_ZERO_SSL_ACCOUNT_EMAIL" --server "$NEW_CA_SERVER_URL")
                            if ! "${REGISTER_ZERO_SSL_CMD_EDIT[@]}"; then
                                log_message ERROR "❌ ZeroSSL 账户注册失败！请检查邮箱地址或稍后重试。"
                                sleep 2
                                continue # 返回编辑菜单
                            fi
                ...
                if [ "$NEED_REISSUE_OR_RELOAD_NGINX" = "y" ]; then
                    ...
                    if [[ "$UPDATE_NOW" =~ ^[Yy]$ ]]; then
                        ...
                        local ACME_INSTALL_COMMAND_ARRAY_EDIT=("$ACME_BIN" --install-cert "${INSTALL_CERT_DOMAINS_ARRAY_EDIT[@]}" --ecc \
                            --key-file "$NEW_KEY_FILE" \
                            --fullchain-file "$NEW_CERT_FILE" \
                            --reloadcmd "systemctl reload nginx")

                        if ! "${ACME_INSTALL_COMMAND_ARRAY_EDIT[@]}"; then
    ...
    while true; do
        ...
        case "$ACCOUNT_CHOICE" in
            ...
            2)
                ...
                local REGISTER_NEW_ACCOUNT_CMD=("$ACME_BIN" --register-account -m "$NEW_ACCOUNT_EMAIL" --server "$REGISTER_CA_SERVER_URL")
                if "${REGISTER_NEW_ACCOUNT_CMD[@]}"; then
            ...
            3)
                ...
                local SET_DEFAULT_ACCOUNT_CMD=("$ACME_BIN" --set-default-account -m "$DEFAULT_ACCOUNT_EMAIL")
                if "${SET_DEFAULT_ACCOUNT_CMD[@]}"; then
...
main_menu() {
    while true; do
        printf "${CYAN}╔═══════════════════════════════════════╗${RESET}\n"
        printf "${CYAN}║     🚀 Nginx/HTTPS 证书管理主菜单     ║${RESET}\n"
        printf "${CYAN}╚═══════════════════════════════════════╝${RESET}\n"
        printf "\n" # 添加空行美化
        printf "${GREEN}  1) 配置新的 Nginx 反向代理和 HTTPS 证书%b\n" "$RESET"
        printf "${GREEN}  2) 查看与管理已配置项目 (域名、端口、证书)%b\n" "$RESET"
        printf "${GREEN}  3) 检查并自动续期所有证书%b\n" "$RESET"
        printf "${GREEN}  4) 管理 acme.sh 账户%b\n" "$RESET"
        printf "${YELLOW}  0) 退出%b\n" "$RESET"
        printf "${CYAN}───────────────────────────────────────%b\n" "$RESET"
