# =============================================================
# 🚀 Nginx 反向代理 + HTTPS 证书管理助手 (v4.27.0 - 日志轮转与底层审查)
# =============================================================
# 作者：Shell 脚本专家
# 描述：自动化管理 Nginx 反代配置与 SSL 证书，支持 TCP 负载均衡、TLS卸载与泛域名智能复用
# 版本历史：
#   v4.27.0 - 注入日志切割守护 (Logrotate)，修复临时文件残留，优化 CF 防御判定逻辑
#   v4.26.0 - 实装外部代理(S-UI/Xray)自动重载组件，重构 CF IP 库更新逻辑形成安全闭环
#   v4.25.0 - 证书详情深度中文化并展示验证方式，列表显示剩余天数，优化灾备面板文案释义
...
initialize_environment() {
    ...
    if [ ! -f "/etc/nginx/conf.d/gzip_optimize.conf" ]; then
        log_message INFO "正在注入 Nginx 全局 Gzip 静态压缩优化配置..."
...
_handle_backup_restore() {
    echo ""
    _render_menu "维护选项与灾备工具" \
        "1. 备份与恢复面板 (数据层: 对全站进行打包，或回滚过去的 projects.json 数据)" \
        "2. 重建所有 HTTP 配置 (应用层: 根据当前的 JSON 数据记录，从零重新生成丢失的 Nginx .conf 配置文件)" \
        "3. 修复定时任务 (系统层: 自动诊断并修复缺失的定时续期进程)"
        ...
        1)
            ...
            case "$bc" in
                1)
                    ...
                    else log_message ERROR "备份失败。"; fi ;;
                2)
...
_manage_cron_jobs() {
    ...
    echo -e " 1. acme.sh 原生续期守护进程 : $( [ $has_acme -eq 1 ] && echo -e "${GREEN}正常运行${NC}" || echo -e "${RED}缺失${NC}" )"
    echo -e " 2. 本面板容错接管守护进程   : $( [ $has_manager -eq 1 ] && echo -e "${GREEN}正常运行${NC}" || echo -e "${RED}缺失${NC}" )"
    echo ""
...
_issue_and_install_certificate() {
    local json="${1:-}"
    if [[ -z "$json" ]] || [[ "$json" == "null" ]]; then log_message WARN "未收到有效配置信息。"; return 1; fi

    local domain=$(echo "$json" | jq -r .domain)
    if [[ -z "$domain" || "$domain" == "null" ]]; then log_message ERROR "内部错误: 域名为空。"; return 1; fi

    local method=$(echo "$json" | jq -r .acme_validation_method)
    
    if [ "$method" == "reuse" ]; then
        log_message SUCCESS "检测到复用关联记录，直接绑定现有证书，跳过申请流程。"
        return 0
    fi
        ...
        if [ "$provider" = "dns_cf" ]; then
            if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
                ...
                if [[ -n "$saved_t" && -n "$saved_a" ]]; then
                    log_message INFO "检测到已保存 Cloudflare 凭证 (Token: $(_mask_string "$saved_t"))"
                    if _confirm_action_or_exit_non_interactive "是否复用已保存的凭证？"; then use_saved="true"; fi
                fi
            ...
            if [ "$IS_INTERACTIVE_MODE" = "true" ]; then
                ...
                if [[ -n "$saved_k" && -n "$saved_s" ]]; then
                    log_message INFO "检测到已保存 Aliyun 凭证 (Key: $(_mask_string "$saved_k"))"
                    if _confirm_action_or_exit_non_interactive "是否复用已保存的凭证？"; then use_saved="true"; fi
                fi
                if [ "$use_saved" = "false" ]; then
                    local k; if ! k=$(_prompt_secret "请输入新的 Ali_Key"); then return 1; fi
                    local s; if ! s=$(_prompt_secret "请输入新的 Ali_Secret"); then return 1; fi
...
EOF
                ...
                log_message WARN "检测到 80 端口被 $temp_svc 占用。"
                if [ "$IS_INTERACTIVE_MODE" = "false" ]; then
                    port_conflict="true"; log_message INFO "Cron 模式自动操作: 临时停止 $temp_svc 以释放端口。"
                else
                    if _confirm_action_or_exit_non_interactive "是否临时停止 $temp_svc 以释放端口?"; then port_conflict="true"; fi
                fi
                if [ "$port_conflict" == "true" ]; then
                    log_message INFO "停止 $temp_svc ..."; systemctl stop "$temp_svc"
                    trap "echo; log_message WARN '中断，恢复 $temp_svc ...'; systemctl start $temp_svc; _cleanup; exit 130" INT TERM
                fi
                cmd+=("--standalone")
            fi
        else
    ...
    # 修复：规范化 mktemp，确保 trap _cleanup 能根据 "acme_cmd_log.*" 正确销毁
    ...
    if [ $ret -ne 0 ]; then
        echo -e "\n"; log_message ERROR "申请失败: $domain"; cat "$log_temp"; local err_log=$(cat "$log_temp"); rm -f "$log_temp"
        
        if [[ "$method" == "http-01" && "$port_conflict" == "true" ]]; then
            log_message INFO "重启 $temp_svc ..."; systemctl start "$temp_svc"; trap '_on_int' INT TERM
        fi
        if [[ "$err_log" == *"retryafter"* ]]; then
            echo -e "\n${RED}检测到 CA 限制 (retryafter)${NC}"
            if _confirm_action_or_exit_non_interactive "是否切换到 Let's Encrypt 并重试?"; then
                "$ACME_BIN" --set-default-ca --server letsencrypt
                json=$(echo "$json" | jq '.ca_server_url = "https://acme-v02.api.letsencrypt.org/directory"')
                _issue_and_install_certificate "$json"; return $?
            fi
        fi
    ...
    if ! "${inst[@]}"; then log_message ERROR "安装失败: $domain"; unset CF_Token CF_Account_ID Ali_Key Ali_Secret; return 1; fi
...
_gather_project_details() {
    ...
    if [ -z "$domain" ]; then
        if ! domain=$(_prompt_user_input_with_validation "主域名" "" "^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$" "格式无效，请输入正确的域名" "false"); then exec 1>&3; return 1; fi
    fi
    ...
    if [ "$is_cert_only" == "false" ]; then
        # 修复：移除之前的强关联 CF 判断，改为基于原有配置进行平滑追问，避免误伤未套用 CDN 的源站
        local cf_strict_default="n"
        [ "$cf_strict" == "y" ] && cf_strict_default="y"
        
        if _confirm_action_or_exit_non_interactive "是否开启 Cloudflare 严格安全防御 (仅允许 CF 节点访问)? [当前: ${cf_strict_default}]"; then
            cf_strict="y"
        else
            cf_strict="n"
        fi
        ...
        if [ "$skip_cert" == "false" ]; then
            ...
            echo -e "${YELLOW}因为您申请的是独立证书，当 60 天后发生自动续期时，外部程序并不知道。${NC}" >&2
            echo -e "${YELLOW}为了让外部程序加载新证书，您必须为其绑定一个自动重启指令：${NC}" >&2
            
            local -a hook_opts=("1. S-UI / 3x-ui / x-ui (执行: systemctl restart x-ui)" "2. V2Ray 原生服务 (执行: systemctl restart v2ray)" "3. Xray 原生服务 (执行: systemctl restart xray)" "4. Nginx 服务 (执行: systemctl reload nginx)" "5. 手动输入自定义的 Shell 重启命令" "6. 跳过 (不绑定重载)")
            _render_menu "自动重启预设方案" "${hook_opts[@]}" >&2
            local hk; while true; do hk=$(_prompt_for_menu_choice_local "1-6"); [ -n "$hk" ] && break; done
            case "$hk" in
                1) reload_cmd="systemctl restart x-ui" ;;
...
_display_projects_list() {
    ...
    printf "${BOLD}%-4s %-12s %-12s %-18s %-s${NC}\n" "ID" "状态(余天)" "下期计划" "目标" "域名"
    echo "────────────────────────────────────────────────────────────────────────"
    local idx=0
    echo "$json" | jq -c '.[]' | while read -r p; do
        ...
        if [[ -f "$cert" ]]; then
            ...
            if (( days < 0 )); then status_str="过期(${days}d)"; status_color="$RED"
            elif (( days <= 30 )); then status_str="急需(${days}d)"; status_color="$YELLOW"
            else status_str="正常(${days}d)"; status_color="$GREEN"; fi
        else status_str="未安装  "; fi
        printf "%-4d ${status_color}%-12s${NC} %-12s %-18s %-s\n" "$idx" "$status_str" "$renew_date" "${target_str:0:18}" "${domain}"
    done; echo ""
}
...
_draw_dashboard() {
    ...
    echo -e "\n${GREEN}╭────────────────────────────────────────────────────────────────────────╮${NC}"
    echo -e "${GREEN}│${NC}                   ${BOLD}Nginx 管理面板 v4.27.0${NC}                   ${GREEN}│${NC}"
    echo -e "${GREEN}╰────────────────────────────────────────────────────────────────────────╯${NC}"
...
_handle_renew_cert() { local d="${1:-}"; local p=$(_get_project_json "$d"); [ -z "$p" ] && { log_message ERROR "不存在"; return; }; _issue_and_install_certificate "$p" && control_nginx reload; press_enter_to_continue; }
...
_handle_modify_renew_settings() {
    ...
    echo -e "\n${CYAN}--- 修改证书续期设置: $d ---${NC}"
    echo -e "${YELLOW}说明: 仅修改本地配置参数。新设置将在系统检测到证书快到期、触发自动续期时接管生效。${NC}"
    ...
    local method=$(echo "$cur" | jq -r '.acme_validation_method // "http-01"')
    local provider=$(echo "$cur" | jq -r '.dns_api_provider // ""')
    case "$v_choice" in
    ...
    local new_json=$(echo "$cur" | jq \
        --arg cu "$ca_server" --arg cn "$ca_name" --arg m "$method" --arg dp "$provider" \
        '.ca_server_url=$cu | .ca_server_name=$cn | .acme_validation_method=$m | .dns_api_provider=$dp')

    if _save_project_json "$new_json"; then log_message SUCCESS "证书续期设置已更新，将在证书快到期时自动应用。"
    else log_message ERROR "保存配置失败。"; fi
    press_enter_to_continue
}
...
_handle_cert_details() { 
    ...
    if [ -f "$cert" ]; then 
        ...
        local method=$(echo "$cur" | jq -r '.acme_validation_method // "未知"')
        local provider=$(echo "$cur" | jq -r '.dns_api_provider // ""')
        local method_zh="未知"
        case "$method" in
            "http-01") method_zh="HTTP 网站根目录验证 (Webroot/Standalone)" ;;
            "dns-01")  method_zh="DNS API 验证 (提供商: ${provider:-未知})" ;;
            "reuse")   method_zh="泛域名证书智能复用 (挂载型)" ;;
        esac
...
check_and_auto_renew_certs() {
    ...
    jq -c '.[]' "$PROJECTS_METADATA_FILE" | while read -r p; do
        ...
        if [ "$m" == "reuse" ]; then
            echo -e "${CYAN}跟随主泛域证书自动更新，跳过独立检测${NC}"
            continue
        fi

        if [ ! -f "$f" ] || ! openssl x509 -checkend $((RENEW_THRESHOLD_DAYS * 86400)) -noout -in "$f"; then
            echo -e "${YELLOW}即将到期，开始续期...${NC}"
            if _issue_and_install_certificate "$p"; then success=$((success+1)); echo -e "   ${GREEN}续期成功${NC}"; else fail=$((fail+1)); echo -e "   ${RED}续期失败 (查看日志)${NC}"; fi
        else echo -e "${GREEN}有效期充足${NC}"; fi
    done
    control_nginx reload; log_message INFO "批量续期结果: $success 独立续期成功, $fail 失败。"
}
...
configure_nginx_projects() {
    ...
    if [ "$mode" != "cert_only" ]; then
        local domain=$(echo "$json" | jq -r .domain)
        if _write_and_enable_nginx_config "$domain" "$json"; then control_nginx reload; log_message SUCCESS "配置已生成并加载。"; else log_message ERROR "Nginx 配置生成失败。"; return; fi
    fi
    _save_project_json "$json"; log_message SUCCESS "配置已保存。"
    local domain=$(echo "$json" | jq -r .domain)
...
main_menu() {
    while true; do
        ...
        local c; if ! c=$(_prompt_for_menu_choice_local "1-9" "true"); then break; fi
        case "$c" in
