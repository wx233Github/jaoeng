#!/bin/bash
# =============================================================
# 🚀 VPS 一键安装入口脚本 (v50.0 - flock 锁与原子退出版)
# =============================================================
...
# --- 主程序逻辑封装 ---
# 将所有核心逻辑封装在一个函数中，以便被 flock 安全地调用
main_logic() {
    # --- 颜色定义 ---
    ...
    self_update() { 
        ...
        local temp_script="/tmp/install.sh.tmp"; if _download_self "$temp_script"; then 
            if ! cmp -s "$SCRIPT_PATH" "$temp_script"; then 
                ...
                # 关键修复: 在重启前，解除 EXIT 陷阱，避免打印第一个“脚本已退出”
                trap - EXIT
    ...
    uninstall_script() {
        ...
        if [[ "$choice" == "yes" ]]; then
            log_info "开始卸载...";
            # flock 会自动释放锁，无需手动 release_lock
    ...
    # --- 主程序启动逻辑 ---
    # 只有在 curl | bash 模式下，才需要执行安装和首次启动逻辑
    if [[ ! -f "${CONFIG[install_dir]}/install.sh" ]]; then
        log_info "首次运行，正在执行安装..."
        load_config # 加载默认配置
        save_entry_script
        setup_shortcut
        # 首次安装后，强制刷新配置以确保一致性
    ...
    load_config
    log_info "脚本启动 (v50.0 - flock 锁与原子退出版)"
    check_and_install_dependencies
...
# --- [核心改造]: 脚本最终启动点 ---
# 使用子 shell 和 flock 来包裹主逻辑的执行
(
    # -n: 如果不能立即获取锁，则失败退出并给出提示
    # 200: 这是一个用户选择的文件描述符，用于 flock
    flock -n 200 || { echo -e "\033[0;33m[警告] 检测到另一脚本实例正在运行，退出。\033[0m" >&2; exit 1; }
    
    # 如果获取锁成功，则执行主逻辑
    main_logic "$@"
    
) 200>"/tmp/vps_install_modules.lock" # flock 将会锁定这个文件
