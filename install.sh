#!/bin/bash
# =============================================================
# 🚀 VPS 一键安装入口脚本 (v71.0 - Self-Ensured Bash Environment)
# =============================================================

# --- [Guard]: Ensure this script is run with bash, not sh/dash ---
# If BASH_VERSION is not set, it means we are not running in bash.
# The guard variable prevents infinite re-execution loops.
if [ -z "$BASH_VERSION" ] && [ -z "$_BASH_GUARD_" ]; then
    # Check if /bin/bash exists and is executable
    if [ -x "/bin/bash" ]; then
        # Re-execute the script using /bin/bash, passing all arguments.
        # The guard variable ensures this happens only once.
...
# Unset the guard for child processes if any
unset _BASH_GUARD_
...
execute_module() {
    ...
    # We must explicitly call `bash` here to ensure sub-scripts run in bash
...
generate_line() { local len=$1; local char="─"; local i=0; local line=""; while (( i < len )); do line="$line$char"; ((i++)); done; echo "$line"; }
...
display_menu() {
    ...
    line_width=$(_get_visual_width "$main_title_text"); if (( line_width > max_width )); then max_width=$line_width; fi
    
    local -a items
    readarray -t items < <(jq -r '.items[] | ((.icon // "›") + "\t" + .name)' <<< "$menu_json")
    ...
    local i=1
    for item in "${items[@]}"; do
        local icon; icon=$(echo -e "$item" | cut -f1)
        local name; name=$(echo -e "$item" | cut -f2-)
