...
script_main() {
    ...
    # --- [核心改造 2/3]: 移除有问题的 exec 重定向 ---
    # 旧的 setup_logging 函数被移除。日志记录由脚本末尾的管道处理。
    # 保留此函数用于创建日志文件，确保权限正确。
    setup_logging() {
    ...
    save_entry_script() { 
        ...
        local temp_path="/tmp/install.sh.self"; if ! _download_self "$temp_path"; then 
            if [[ "$0" == "bash" || -z "$0" ]]; then log_error "无法自动保存。请使用 'bash <(curl ...)' 或先下载再执行的方式。"; else sudo cp "$0" "$SCRIPT_PATH"; fi; 
    ...
    execute_module() {
        ...
        local exit_code=0
        # 使用 read -p < /dev/tty 确保在管道中也能从键盘读取输入
    ...
    display_menu() {
        ...
        local config_path="${CONFIG[install_dir]}/config.json"; local header_text="🚀 VPS 一键安装入口 (v14.0)"; if [ "$CURRENT_MENU_NAME" != "MAIN_MENU" ]; then header_text="🛠️ ${CURRENT_MENU_NAME//_/ }"; fi
        ...
        if [ "$AUTO_YES" == "true" ]; then
            ...
            # 使用 < /dev/tty 确保在 'curl | bash' 模式下能从键盘读取输入
    ...
    main() {
        ...
        log_info "脚本启动 (v14.0 - 同步管道与健壮日志版)"
...
# 将所有业务逻辑的 stdout 和 stderr 都通过管道传给 tee
# tee 会同时输出到屏幕和日志文件
script_main "$@" 2>&1 | tee -a "$LOG_FILE"

# 使用 PIPESTATUS[0] 来获取 script_main 的真实退出码，而不是 tee 的
# 这确保了脚本在出错时能返回一个非零的退出状态
exit "${PIPESTATUS[0]}"
