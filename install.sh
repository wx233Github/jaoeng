#!/bin/bash
# =============================================================
# 🚀 VPS 一键安装入口脚本 (v8.4 - sudo 修复与健壮解析版)
# =============================================================
...
save_entry_script() { sudo mkdir -p "${CONFIG[install_dir]}"; local SCRIPT_PATH="${CONFIG[install_dir]}/install.sh"; log_info "正在检查并保存入口脚本..."; local temp_path="/tmp/install.sh.self"; if ! with_lock _download_self "$temp_path"; then if [[ "$0" == /dev/fd/* || "$0" == "bash" ]]; then log_error "无法自动保存入口脚本。"; else sudo cp "$0" "$SCRIPT_PATH"; fi; else sudo mv "$temp_path" "$SCRIPT_PATH"; fi; sudo chmod +x "$SCRIPT_PATH"; }
# --- 【核心修复点】 ---
setup_shortcut() { 
    ...
    if [ ! -L "$BIN_DIR/jb" ] || [ "$(readlink "$BIN_DIR/jb")" != "$SCRIPT_PATH" ]; then 
        sudo ln -sf "$SCRIPT_PATH" "$BIN_DIR/jb"; 
        log_success "快捷指令 'jb' 已创建。"; 
    fi; 
}
self_update() { local SCRIPT_PATH="${CONFIG[install_dir]}/install.sh"; if [[ "$0" != "$SCRIPT_PATH" ]]; then return; fi; log_info "正在检查入口脚本更新..."; local temp_script="/tmp/install.sh.tmp"; if with_lock _download_self "$temp_script"; then if ! cmp -s "$SCRIPT_PATH" "$temp_script"; then log_info "检测到新版本，正在自动更新..."; sudo mv "$temp_script" "$SCRIPT_PATH"; sudo chmod +x "$SCRIPT_PATH"; log_success "脚本已更新！正在重新启动..."; exec sudo bash "$SCRIPT_PATH" "$@"; fi; rm -f "$temp_script"; else log_warning "无法连接 GitHub 检查更新。"; fi; }
download_module_to_cache() { sudo mkdir -p "$(dirname "${CONFIG[install_dir]}/$1")"; local script_name="$1"; local local_file="${CONFIG[install_dir]}/$script_name"; local url="${CONFIG[base_url]}/$script_name"; local http_code; http_code=$(curl -sL --connect-timeout 5 --max-time 60 "$url" -o "$local_file" -w "%{http_code}"); if [ "$http_code" -eq 200 ] && [ -s "$local_file" ]; then return 0; else sudo rm -f "$local_file"; log_warning "下载 [$script_name] 失败 (HTTP: $http_code)。"; return 1; fi; }
_update_all_modules() {
...
execute_module() {
    ...
    log_info "您选择了 [$display_name]"; if [ ! -f "$local_path" ]; then log_info "本地未找到模块，正在下载..."; if ! with_lock download_module_to_cache "$script_name"; then log_error "下载模块失败。"; return 1; fi; fi
...
display_menu() {
    local config_path="${CONFIG[install_dir]}/config.json"
    local header_text="🚀 VPS 一键安装入口 (v8.4)"; if [ "$CURRENT_MENU_NAME" != "MAIN_MENU" ]; then header_text="🛠️ ${CURRENT_MENU_NAME//_/ }"; fi
...
main() {
    ...
    load_config; setup_logging; log_info "脚本启动 (v8.4)"; check_and_install_dependencies
    local SCRIPT_PATH="${CONFIG[install_dir]}/install.sh"
    if [ ! -f "$SCRIPT_PATH" ]; then with_lock save_entry_script; fi
    with_lock setup_shortcut; self_update
    while true; do clear 2>/dev/null || true; display_menu; local exit_code=0
