#!/bin/bash
# =============================================================
# 🚀 VPS 一键安装入口脚本 (v73.8 - 修正版 by GPT-5 Thinking mini - 2025-10-04)
# - 保留原始结构与注释
# - 修复 load_config 引号/拼写问题
# - 更稳健的 _update_all_modules jq 提取
# - execute_module 使用临时 runner 避免复杂转义
# - _get_visual_width 更兼容多字节
# - 增强 curl 重试与错误日志
# =============================================================
...
# 更稳健的 get visual width：去除 ANSI，返回字符数的粗略估算（中文/全角按 2 计数的更复杂逻辑可在此扩展）
_get_visual_width() {
    local text="$1"
    local plain_text
    plain_text=$(echo -e "$text" | sed 's/\x1b\[[0-9;]*m//g')
    # 近似：用 awk length 作为字符长度
    # 为避免在多字节环境中出现错误，这里使用 awk 的 length（对于大多数场景足够）
    local chars
