...
if [ "$0" != "$FINAL_SCRIPT_PATH" ]; then
    ...
    if [ ! -f "$FINAL_SCRIPT_PATH" ] || [ ! -f "$CONFIG_PATH" ] || [ ! -f "$UTILS_PATH" ] || [ "${FORCE_REFRESH}" = "true" ]; then
        ...
        echo_info "正在创建/更新快捷指令 'jb'..."; BIN_DIR="/usr/local/bin"; sudo bash -c "ln -sf '$FINAL_SCRIPT_PATH' '$BIN_DIR/jb'"; echo_success "安装/更新完成！"
    fi
...
load_config() {
    CONFIG_FILE="${CONFIG[install_dir]}/config.json"; if [ -f "$CONFIG_FILE" ] && command -v jq &>/dev/null; then
        ...
        CONFIG[dependencies]="$(jq -r '.dependencies.common // "curl cmp ln dirname flock jq"' "$CONFIG_FILE")"; CONFIG[lock_file]="$(jq -r '.lock_file // "/tmp/vps_install_modules.lock"' "$CONFIG_FILE")"; CONFIG[enable_auto_clear]=$(jq -r '.enable_auto_clear // false' "$CONFIG_FILE")"; CONFIG[timezone]=$(jq -r '.timezone // "Asia/Shanghai"' "$CONFIG_FILE")"
    fi
}
...
self_update() { 
    local SCRIPT_PATH="${CONFIG[install_dir]}/install.sh"; if [ "$0" != "$SCRIPT_PATH" ]; then return; fi; local temp_script="/tmp/install.sh.tmp.$$"; if ! _download_file "install.sh" "$temp_script"; then log_warn "主程序 (install.sh) 更新检查失败 (无法连接)。"; rm -f "$temp_script"; return; fi
    if ! cmp -s "$SCRIPT_PATH" "$temp_script"; then log_success "主程序 (install.sh) 已更新。正在无缝重启..."; sudo mv "$temp_script" "$SCRIPT_PATH"; sudo chmod +x "$SCRIPT_PATH"; flock -u 200; rm -f "${CONFIG[lock_file]}"; trap - EXIT; exec sudo -E bash "$SCRIPT_PATH" "$@"; fi; rm -f "$temp_script"
}
download_module_to_cache() { 
    ...
    if [ -f "$local_file" ] && cmp -s "$local_file" "$tmp_file"; then rm -f "$tmp_file"; else log_success "模块 (${script_name}) 已更新。"; sudo mkdir -p "$(dirname "$local_file")"; sudo mv "$tmp_file" "$local_file"; fi
}
_update_core_files() {
    local temp_utils="/tmp/utils.sh.tmp.$$"; if _download_file "utils.sh" "$temp_utils"; then if [ ! -f "$UTILS_PATH" ] || ! cmp -s "$UTILS_PATH" "$temp_utils"; then log_success "核心工具库 (utils.sh) 已更新。"; sudo mv "$temp_utils" "$UTILS_PATH"; sudo chmod +x "$UTILS_PATH"; else rm -f "$temp_utils"; fi; else log_warn "核心工具库 (utils.sh) 更新检查失败。"; fi
}
_update_all_modules() {
...
confirm_and_force_update() {
    ...
    if [ "$choice" = "yes" ]; then
        log_info "开始强制完全重置..."; declare -A core_files_to_reset=( ["主程序"]="install.sh" ["工具库"]="utils.sh" ["配置文件"]="config.json" )
        for name in "${!core_files_to_reset[@]}"; do local file_path="${core_files_to_reset[$name]}"; log_info "正在强制更新 ${name}..."; local temp_file="/tmp/$(basename "$file_path").tmp.$$"; if ! _download_file "$file_path" "$temp_file"; then log_err "下载最新的 ${name} 失败。"; continue; fi; sudo mv "$temp_file" "${CONFIG[install_dir]}/${file_path}"; log_success "${name} 已重置为最新版本。"; done
        log_info "正在恢复核心脚本执行权限..."; sudo chmod +x "${CONFIG[install_dir]}/install.sh" "${CONFIG[install_dir]}/utils.sh"; log_success "权限已恢复。"
        _update_all_modules; log_success "强制重置完成！"; log_info "脚本将在2秒后自动重启以应用所有更新..."; sleep 2
        flock -u 200; rm -f "${CONFIG[lock_file]}"; exec sudo -E bash "$FINAL_SCRIPT_PATH"
    else log_info "操作已取消."; fi; return 10 
}
uninstall_script() {
...
execute_module() {
    ...
    if [ "$module_config_json" != "null" ]; then
        ...
        while IFS=$'\t' read -r key value; do if [ -n "$key" ]; then local key_upper; key_upper=$(echo "$key" | tr '[:lower:]' '[:upper:]'); env_exports+=$(printf "export %s_CONF_%s=%q;" "$prefix" "$key_upper" "$value"); fi; done < <(echo "$module_config_json" | jq -r "$jq_script")
    fi
    local exit_code=0; local extra_args_str; extra_args_str=$(_quote_args "$@"); sudo bash -c "$env_exports bash '$local_path' $extra_args_str" < /dev/tty || exit_code=$?
    if [ "$exit_code" -eq 0 ]; then log_success "模块 [$display_name] 执行完毕."; elif [ "$exit_code" -eq 10 ]; then log_info "已从 [$display_name] 返回."; else log_warn "模块 [$display_name] 执行出错 (码: $exit_code)."; fi; return $exit_code
}
display_menu() {
    ...
    local menu_content_str=""; local i=1
...
process_menu_selection() {
    ...
    if [ -z "$choice" ]; then if [ "$CURRENT_MENU_NAME" = "MAIN_MENU" ]; then exit 0; else CURRENT_MENU_NAME="MAIN_MENU"; return 10; fi; fi; if ! [[ "$choice" =~ ^[0-9]+$ ]] || [ "$choice" -lt 1 ] || [ "$choice" -gt "$menu_len" ]; then log_warn "无效选项."; return 10; fi
    local item_json; item_json=$(echo "$menu_json" | jq -r --argjson idx "$(expr $choice - 1)" '.items[$idx]'); if [ -z "$item_json" ] || [ "$item_json" = "null" ]; then log_warn "菜单项配置无效或不完整。"; return 10; fi
