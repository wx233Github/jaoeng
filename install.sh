#!/bin/bash
# =============================================================
# 🚀 通用工具函数库 (v4.6.28-DeepDebugRender - 深度调试渲染函数)
# - [新增调试] 在 `_render_menu` 函数内部添加详细日志，以诊断其是否被意外调用以及接收的参数。
...
_render_menu() {
    ...
    log_info "DEBUG: Inside _render_menu. Received title: '$menu_title'" # <-- Added debug
    log_info "DEBUG: Inside _render_menu. Received ${#items[@]} items." # <-- Added debug
    if [ "${#items[@]}" -gt 0 ]; then
        log_info "DEBUG: Inside _render_menu. First item: '${items[0]}'" # <-- Added debug
    fi
