#!/bin/bash
# =============================================================
# 🚀 VPS 一键安装入口脚本 (v73.3 - New Status Panel UI)
# =============================================================
...
_render_main_status_panel() {
    ...
    local certs_status="○ Certs       ：$(command -v ~/.acme.sh/acme.sh &>/dev/null && echo -e "${GREEN}🟢 已安装${NC}" || echo -e "${RED}🔴 未配置${NC}")"
    
    echo -e "${CYAN}${line}${NC}"
    echo -e "● VPS 工具中心  ·  状态：[${GREEN}正常 ✓${NC}]"
    echo -e "${CYAN}${line}${NC}"
...
display_menu() {
    ...
    local config_path="${CONFIG[install_dir]}/config.json"; local menu_json; menu_json=$(jq -r --arg menu "$CURRENT_MENU_NAME" '.menus[$menu]' "$config_path")
