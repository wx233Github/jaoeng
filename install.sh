...
# 如果不是（例如，通过 curl | bash 执行），则进入“安装器模式”。
if [[ "$0" != "$FINAL_SCRIPT_PATH" ]]; then
    ...
    if ! sudo curl -fsSL "${BASE_URL}/install.sh?_=$(date +%s)" -o "$FINAL_SCRIPT_PATH"; then
    ...
    # exec 执行后，此处的代码永远不会被执行
fi
# --- 启动器结束。从这里开始，是作为“主程序”运行的逻辑 ---
...
self_update() { 
    ...
    # self_update 只在主程序模式下被调用，所以 $0 总是 SCRIPT_PATH
    log_info "检查主脚本更新..."; 
...
display_menu() {
    ...
    local config_path="${CONFIG[install_dir]}/config.json"; local header_text="🚀 VPS 一键安装入口 (v47.0)"; if [ "$CURRENT_MENU_NAME" != "MAIN_MENU" ]; then header_text="🛠️ ${CURRENT_MENU_NAME//_/ }"; fi
...
main() {
    ...
    # setup_logging # 不再需要，但保留函数以防未来使用
