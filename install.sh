...
check_and_install_dependencies() {
    ...
    if [ ${#missing_deps[@]} -gt 0 ]; then
        log_warning "缺少核心依赖: ${missing_deps[*]}"; local pm; pm=$(command -v apt-get &>/dev/null && echo "apt" || (command -v dnf &>/dev/null && echo "dnf" || (command -v yum &>/dev/null && echo "yum" || echo "unknown")))
        if [ "$pm" == "unknown" ]; then log_error "无法检测到包管理器, 请手动安装: ${missing_deps[*]}"; fi
        read -p "$(echo -e "${YELLOW}是否尝试自动安装? (y/N): ${NC}")" choice
        if [[ "$choice" =~ ^[Yy]$ ]]; then
            log_info "正在使用 $pm 安装..."; local update_cmd=""; if [ "$pm" == "apt" ]; then update_cmd="sudo apt-get update"; fi
            if ! $update_cmd && sudo $pm install -y ${missing_deps[@]}; then log_error "依赖安装失败。"; fi; log_success "依赖安装完成！"
...
main() {
    ...
    CONFIG[dependencies]='curl cmp ln dirname flock jq'; CONFIG[enable_auto_clear]="false"; CONFIG[timezone]="Asia/Shanghai"
    if ! command -v jq &>/dev/null; then check_and_install_dependencies; fi
    load_config; setup_logging; log_info "脚本启动 (v9.9)"; check_and_install_dependencies
