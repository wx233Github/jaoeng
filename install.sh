#!/bin/bash
# =============================================================
# 🚀 Docker 自动更新助手 (v4.4.3 - Interactive Restart)
# =============================================================
...
notification_menu() {
    while true; do if [ "${JB_ENABLE_AUTO_CLEAR}" = "true" ]; then clear; fi
        ...
        case "$choice" in
            1) 
                _configure_telegram; save_config
                _prompt_and_restart_watchtower_if_needed
                press_enter_to_continue 
                ;;
            2) 
                _configure_email; save_config
                # Email 配置不会影响 Watchtower 容器本身，因此不需要重启提示
                press_enter_to_continue 
                ;;
            3) 
                if [ -z "$TG_BOT_TOKEN" ] || [ -z "$TG_CHAT_ID" ]; then log_warn "请先配置至少一种通知方式。"; else log_info "正在发送测试..."; send_notify "这是一条来自 Docker 助手 v${SCRIPT_VERSION} 的*测试消息*。"; log_info "测试通知已发送。"; fi
                press_enter_to_continue 
                ;;
            4) 
                if confirm_action "确定要清空所有通知配置吗?"; then 
                    TG_BOT_TOKEN=""; TG_CHAT_ID=""; EMAIL_TO=""; save_config
                    log_info "所有通知配置已清空。"
                    _prompt_and_restart_watchtower_if_needed
                else 
                    log_info "操作已取消。"
...
# ... (脚本的其余部分保持不变，为节省篇幅已省略) ...
# ... 您只需将上面的代码与您现有的 Watchtower.sh 文件中对应的 notification_menu 函数及其辅助函数部分进行替换即可 ...
# ... 为了确保完整性，下面提供了完整的脚本 ...
