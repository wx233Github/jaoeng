...
# --- 简化版主菜单渲染函数 (用于调试) ---
render_main_menu() {
    echo "DEBUG: Entering simplified render_main_menu"
    sleep 0.5 # 增加延迟，确保调试信息可见

    local main_menu_title="🖥️ VPS 一键安装脚本 (简化版)"
    local -a menu_lines=(
        "  1. 🐳 Docker"
        "  2. 🌐 Nginx"
        "  3. 🛠️ 常用工具"
    ...
    echo "DEBUG: Exiting simplified render_main_menu"
    sleep 0.5 # 增加延迟
}
...
main() {
    ...
    echo "DEBUG: Calling main_menu"
    main_menu
    echo "DEBUG: Exiting main function"
    exit 0
}
